...
//        Log.i("Active subscriptions count increased to %d", count);
    }

    public void decrementActiveSubscriptionsCount() {
        int count = mActiveSubscriptionsCount.decrementAndGet();
//        Log.i("Active subscriptions count decreased to %d", count);
        if (isIdleNow()) {
//            Log.i("There is no active subscriptions, transitioning to Idle");
