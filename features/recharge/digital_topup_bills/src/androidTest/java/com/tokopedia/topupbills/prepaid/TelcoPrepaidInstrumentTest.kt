...
class TelcoPrepaidInstrumentTest {
    ...
    fun stubAllExternalIntents() {
        ...
        mockkObject(CommonTopupBillsUtil)
        every { CommonTopupBillsUtil.isSeamlessFavoriteNumber(any()) } returns false
        every { CommonTopupBillsUtil.getApplinkFavoriteNumber(any()) } returns ApplinkConsInternalDigital.FAVORITE_NUMBER
...
//        click_phonebook_and_clear()
    ...
    fun validate_click_on_contact_picker_and_list_fav_number() {
        ...
        onView(withId(R.id.telco_field_input_number)).check(matches(AnyOf.anyOf(withText(VALID_PHONE_BOOK), withText(VALID_PHONE_BOOK_RAW))))
    }
