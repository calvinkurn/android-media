...
class TopupBillsSearchNumberActivityTest {
    ...
    var mActivityRule: IntentsTestRule<TopupBillsSearchNumberActivity> =
        object : IntentsTestRule<TopupBillsSearchNumberActivity>(TopupBillsSearchNumberActivity::class.java) {
            override fun getActivityIntent(): Intent {
                val favNumbers = arrayListOf<TopupBillsSearchNumberDataModel>()
                val targetContext = InstrumentationRegistry.getInstrumentation().targetContext
                return Intent(targetContext, TopupBillsSearchNumberActivity::class.java).apply {
                    putExtra(TopupBillsSearchNumberActivity.EXTRA_CLIENT_NUMBER, "")
                    putExtra(TopupBillsSearchNumberActivity.EXTRA_CLIENT_NUMBER_TYPE, ClientNumberType.TYPE_INPUT_TEL.value)
    ...
    fun click_clear_on_search_number_view() {
        onView(withId(com.tokopedia.unifycomponents.R.id.searchbar_textfield)).perform(ViewActions.typeText(VALID_PHONE_NUMBER), ViewActions.closeSoftKeyboard())
        ...
        onView(withId(com.tokopedia.unifycomponents.R.id.searchbar_icon)).perform(ViewActions.click())
        onView(withId(com.tokopedia.unifycomponents.R.id.searchbar_textfield)).check(matches(withText("")))
    }
