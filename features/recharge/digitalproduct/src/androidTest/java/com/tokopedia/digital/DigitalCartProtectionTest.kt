...
class DigitalCartProtectionTest {
    @get:Rule
    ...
    fun testCartProtection() {
        ...
        Espresso.onView(ViewMatchers.withText(("Proteksi Layar Retak"))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        Espresso.onView(ViewMatchers.withText(("Beli pulsa dapat perlindungan layar retak 30 hari, cukup tambah Rp.3.038/transaksi"))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
    ...
    companion object {
        const val PATH_RESPONSE_CART_PROTECTION = "response_mock_data_cart_protection.json"
    }
}