...
class DigitalCartInputPriceTest {
    @get:Rule
    ...
    fun testCartActivateAutodebet() {
        ...
        setupRestMockResponse {
            addMockResponse(DigitalQueries.KEY_QUERY_CART,
                    ResourceUtils.getJsonFromResource(PATH_RESPONSE_CART_INPUT_PRICE),
        ...
        //Input nominal price
        Espresso.onView(ViewMatchers.withId((R.id.input_price_info))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        Espresso.onView(ViewMatchers.withId((R.id.input_price_edittext))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        Espresso.onView(ViewMatchers.withId((R.id.error_input_price))).check(ViewAssertions.matches(IsNot(ViewMatchers.isDisplayed())))
        Espresso.onView(ViewMatchers.withId(R.id.input_price_edittext)).perform(ViewActions.typeText("200000000"), ViewActions.closeSoftKeyboard())
        Espresso.onView(ViewMatchers.withId((R.id.error_input_price))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        Thread.sleep(3000)
        Espresso.onView(ViewMatchers.withId(R.id.input_price_edittext)).perform(ViewActions.clearText(), ViewActions.typeText("20000000"), ViewActions.closeSoftKeyboard())
        Espresso.onView(ViewMatchers.withId((R.id.error_input_price))).check(ViewAssertions.matches(IsNot(ViewMatchers.isDisplayed())))

        //Select Promo Voucher Discount
        Thread.sleep(3000)
    ...
    companion object {
        const val PATH_RESPONSE_CART_INPUT_PRICE = "response_mock_data_cart_empty_input_price.json"
    }
}