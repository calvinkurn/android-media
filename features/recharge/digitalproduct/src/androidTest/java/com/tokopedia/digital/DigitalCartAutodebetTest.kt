...
class DigitalCartAutodebetTest {
    @get:Rule
    ...
    fun testCartActivateAutodebet() {
        ...
        //Autodebet Payment
        Espresso.onView(ViewMatchers.withText(("Aktifkan auto debit untuk tagihan ini"))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        Espresso.onView(ViewMatchers.withText(("Bayar tagihan ini secara otomatis, lebih mudah dan hemat waktu. Yuk mulai berlangganan."))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        Espresso.onView(AllOf.allOf(ViewMatchers.withId((R.id.cb_subscription)), ViewMatchers.isDisplayed())).perform(ViewActions.click())
        Espresso.onView(ViewMatchers.withText(("Kamu harus memasukkan OVO PIN untuk mulai auto debit tagihan ini pada 2 November 2020"))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
