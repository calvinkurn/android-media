...
class DigitalCartEgoldTest {
    ...
    fun testCartEgold() {
        ...
        setupRestMockResponse {
            addMockResponse(KEY_QUERY_CART,
                    ResourceUtils.getJsonFromResource(PATH_RESPONSE_CART_EGOLD),
        ...
        Espresso.onView(ViewMatchers.withText(("Yuk mulai nabung emas"))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        Espresso.onView(ViewMatchers.withText(("Bulatkan tagihan ini dengan Rp 7.250"))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        Intents.intending(IntentMatchers.anyIntent()).respondWith(Instrumentation.ActivityResult(Activity.RESULT_OK, null))
        Espresso.onView(AllOf.allOf(ViewMatchers.withId(R.id.cb_subscription), ViewMatchers.isDisplayed())).perform(ViewActions.click())
        Espresso.onView(AllOf.allOf(ViewMatchers.withId(R.id.ic_more_info), ViewMatchers.isDisplayed())).perform(ViewActions.click())
        Espresso.onView(ViewMatchers.withId((R.id.egold_tooltip))).check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
        Espresso.onView(ViewMatchers.withId(R.id.egold_tooltip)).check(ViewAssertions
                .matches(ViewMatchers.withText("Nominal pembulatan disesuaikan dengan total " +
                        "tagihan setiap transaksi yang otomatis ditabung dan dapat di cek saldonya " +
                        "di Tokopedia Emas (berpartner dengan Pegadaian).")))
    ...
    companion object {
        ...
        const val KEY_QUERY_CANCEL_VOUCHER = "{\"data\":{\"attributes\":{\"identifier\":{\"device_token\":\"eMoRJ2rNOx8:APA91bG5Kd6vC-hGaBOMx-PxXDXGpMzNs1G_b8PgrMNkPhlIw5xkEaqLfuo4SYGM3Z2-ROlgwol51Kg_QLI0-PFic40rHOVC300S7LsRXxRW0H8ZGI22DqVlv7OdQLUBeA3MiCqhR8N5\",\"os_type\":\"1\",\"user_id\":\"108956738\"}}}}"
        const val PATH_RESPONSE_CART_EGOLD = "response_mock_data_cart_egold.json"
    }
}