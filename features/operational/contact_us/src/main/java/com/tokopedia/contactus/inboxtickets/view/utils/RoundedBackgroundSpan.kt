...
class RoundedBackgroundSpan constructor(
    ...
    private val textHeightWrap: Float
) : ReplacementSpan() {
    ...
    override fun draw(
        ...
        canvas.drawText(
            ...
            x + PADDING_X,
            tagBottom - PADDING_Y - textHeightWrap - MAGIC_NUMBER,
            paint
        )
    }
    ...
    private fun getTagWidth(text: CharSequence, start: Int, end: Int, paint: Paint): Int {
        return Math.round(
            PADDING_X + paint.measureText(text.subSequence(start, end).toString()) + PADDING_X
        )
    }
