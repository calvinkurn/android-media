...
sealed class TicketUiEffect {
    ...
    data class GetDetailTicketFailed(val messageError: String = "", val throwable: Throwable? = null): TicketUiEffect()

    data class OnSearchTicketKeyword(val isOnProgress : Boolean = false, val searchKeyword : String= "", val sizeSearch : Int= 0) : TicketUiEffect()
    ...
    data class SendTextMessageFailed(val messageError: String= "", val throwable: Throwable? = null): TicketUiEffect()

    data class OnSendRatingSuccess(val rating : Int, val commentItem : CommentsItem = CommentsItem(), val commentPosition : Int= 0): TicketUiEffect()

    data class OnSendRatingFailed(val messageError: String= "", val throwable: Throwable? = null): TicketUiEffect()

}

data class OnFindKeywordAtTicket(val positionAdapter : Int = 0, val positionKeyword : Int= 0, val status : Int= FIND_KEYWORD)
