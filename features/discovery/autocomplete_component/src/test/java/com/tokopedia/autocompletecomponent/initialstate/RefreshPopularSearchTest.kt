...
internal class RefreshPopularSearchTest: InitialStatePresenterTestFixtures() {
    ...
    private fun `Test Refresh Popular Search`(
        initialStateResponse: InitialStateUniverse,
        refreshPopularSearchResponse: List<InitialStateData>,
    ) {
    ...
    fun `Test refresh popular search data`() {
        val multiplePopularSearchData = multiplePopularResponse.jsonToObject<InitialStateUniverse>()
        ...
        `Then verify visitable list after refresh dynamic initial state data`(
            multiplePopularSearchData.data,
            refreshedPopularSearchData
        )
    }
