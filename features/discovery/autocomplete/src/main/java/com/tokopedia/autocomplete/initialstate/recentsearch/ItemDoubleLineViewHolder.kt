...
import kotlinx.android.synthetic.main.layout_recent_search_double_line_item_autocomplete.view.*

class ItemDoubleLineViewHolder(itemView: View, private val clickListener: InitialStateItemClickListener) : RecyclerView.ViewHolder(itemView) {
    ...
    private fun bindSubtitle(item: BaseItemInitialStateSearch) {
        itemView.recentSearchSubtitle?.setTextAndCheckShow(item.subtitle)
    }

    private fun bindTitle(item: BaseItemInitialStateSearch) {
        itemView.recentSearchTitle?.setTextAndCheckShow(item.title)
    ...
    private fun setTitleMaxLine(item: BaseItemInitialStateSearch) {
        if (item.subtitle.isEmpty()) {
            itemView.recentSearchTitle?.maxLines = 2
        }
    }

    private fun bindLabel(item: BaseItemInitialStateSearch) {
        itemView.recentSearchLabel?.setTextAndCheckShow(item.label)
        if(itemView.recentSearchLabel.text.isNotEmpty()) {
            itemView.recentSearchLabel?.setLabelType(0)
        }
    }
