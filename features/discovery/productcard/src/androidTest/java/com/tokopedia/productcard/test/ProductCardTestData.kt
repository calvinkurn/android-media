...
internal val productCardModelSmallGridList = mutableListOf<ProductCardModel>().also {
    ...
    it.add(3, ProductCardModel(
            ...
            freeOngkir = FreeOngkir(isActive = true, imageUrl = freeOngkirImageUrl),
            hasOptions = true,
            labelGroupList = mutableListOf<Label>().also { labelGroups ->
                labelGroups.add(Label(position = "status", title = "Preorder", type = "darkGrey"))
                labelGroups.add(Label(position = "price", title="Cashback", type = "lightGreen"))
            },
            isTopAds = true
    ))
}