...
open class SimilarProductRecommendationFragment :
    ...
    override fun onProductClick(item: RecommendationItem, layoutType: String?, vararg position: Int) {
        try {
            if (recommendationViewModel.isLoggedIn()) {
                SimilarProductRecommendationTracking.eventClick(item, item.position.toString(), ref, internalRef, recommendationViewModel.userId())
            } else {
                SimilarProductRecommendationTracking.eventClickNonLogin(item, item.position.toString(), ref, internalRef, recommendationViewModel.userId())
            }
    ...
    override fun onProductImpression(item: RecommendationItem) {
        trackingQueue?.let { trackingQueue ->
            if (recommendationViewModel.isLoggedIn()) {
                SimilarProductRecommendationTracking.eventImpression(trackingQueue, item, item.position.toString(), ref, internalRef, recommendationViewModel.userId())
            } else {
                SimilarProductRecommendationTracking.eventImpressionNonLogin(trackingQueue, item, item.position.toString(), ref, internalRef, recommendationViewModel.userId())
            }
        }
    }
