...
class MixLeftComponentViewHolder (itemView: View,
    ...
    override fun bind(element: MixLeftDataModel) {
        ...
        itemView.addOnImpressionListener(element.channelModel)  {
            if (!isCacheData)
                mixLeftComponentListener?.onMixLeftImpressed(element.channelModel, absoluteAdapterPosition)
        }
    }
    ...
    override fun onProductCardImpressed(channelModel: ChannelModel, channelGrid: ChannelGrid, position: Int) {
        if (!isCacheData)
            mixLeftComponentListener?.onProductCardImpressed(channelModel, channelGrid, absoluteAdapterPosition, position)
    }

    override fun onProductCardClicked(channelModel: ChannelModel, channelGrid: ChannelGrid, position: Int, applink: String) {
        mixLeftComponentListener?.onProductCardClicked(channelModel, channelGrid, absoluteAdapterPosition, position, applink)
    }
    ...
    private fun setupBackground(channel: ChannelModel) {
        if (channel.channelBanner.imageUrl.isNotEmpty()) {
            ...
            image.addOnImpressionListener(channel){
                if (!isCacheData)
                    mixLeftComponentListener?.onImageBannerImpressed(channel, absoluteAdapterPosition)
            }
    ...
    private fun setupList(channel: ChannelModel) {
        ...
        listData.add(CarouselEmptyCardDataModel(channel, absoluteAdapterPosition, this, channel.channelBanner.applink))
