...
class CountDrawable(context: Context) : Drawable() {
    ...
    override fun draw(canvas: Canvas) {
        ...
        val centerX = width - radius - CENTER_X_OFFSET
        val centerY = radius - CENTER_Y_OFFSET
        if (count.length <= 2) {
            // Draw badge circle.
            canvas.drawCircle(centerX, centerY, (radius + RADIUS_OFFSET_TWO_CHARACTER).toInt().toFloat(), badgePaint)
        } else {
            canvas.drawCircle(centerX, centerY, (radius + RADIUS_OFFSET_THREE_CHARACTER).toInt().toFloat(), badgePaint)
        }
    ...
    companion object{
         ...
         * from 1f + 5 => 6f
         */
        private const val CENTER_X_OFFSET = 6f
        private const val CENTER_Y_OFFSET = 5
        private const val RADIUS_OFFSET_TWO_CHARACTER = 5.5
        private const val RADIUS_OFFSET_THREE_CHARACTER = 6.5
    }
}
