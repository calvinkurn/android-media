 ...
 * created by Dhaba
 */
class BestSellerProductVariant3Test {
    ...
    fun setUp() {
        ...
        setupGraphqlMockResponse(SearchMockModelConfig(com.tokopedia.search.test.R.raw.search_product_best_seller_variant_3))
    ...
    private fun performUserJourney() {
        ...
        val bestSellerVariant3Position =
            recyclerView.getProductListAdapter().itemList.getVariant3BestSellerProduct()
        Espresso.onView(ViewMatchers.withId(recyclerViewId)).perform(
            RecyclerViewActions.scrollToPosition<RecyclerView.ViewHolder>(
                bestSellerVariant3Position
            )
        )
        Espresso.onView(ViewMatchers.withId(recyclerViewId)).perform(
