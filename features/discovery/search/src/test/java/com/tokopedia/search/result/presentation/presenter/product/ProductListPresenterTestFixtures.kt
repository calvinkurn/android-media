...
internal open class ProductListPresenterTestFixtures {
    ...
    protected val remoteConfig = mockk<RemoteConfig>().also {
        every { it.getBoolean(RemoteConfigKey.ENABLE_GLOBAL_NAV_WIDGET) } returns true
        every { it.getBoolean(RemoteConfigKey.APP_CHANGE_PARAMETER_ROW) } returns false
        every { it.getBoolean(RemoteConfigKey.ENABLE_BOTTOM_SHEET_FILTER) } returns true
        every { it.getBoolean(RemoteConfigKey.ENABLE_TRACKING_VIEW_PORT) } returns true
        // Test this toggle as false until old quick filter is not needed anymore
        every { it.getBoolean(RemoteConfigKey.ENABLE_BOTTOM_SHEET_FILTER_REVAMP) } returns false
    }
