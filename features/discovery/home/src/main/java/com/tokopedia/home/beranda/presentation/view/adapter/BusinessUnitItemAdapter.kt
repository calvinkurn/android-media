...
class BusinessUnitItemAdapter(private val tabIndex: Int, private val tabName: String, private val listenerBusinessTrackerTracker: NewBusinessUnitViewHolder.BusinessUnitItemTrackerListener) : RecyclerView.Adapter<SizeSmallBusinessViewHolder>(){
    ...
    private var listener = object: BusinessUnitItemViewListener{
        ...
        override fun onClicked(position: Int) {
            ...
            listenerBusinessTrackerTracker.onClickTracking(BusinessUnitTracking.getBusinessUnitClick(BusinessUnitTracking.mapToPromotionTracker(element, element.tabName, element.tabPosition+1, positionWidgetOnHome)) as HashMap<String, Any>)
        }

        override fun onImpressed(element: BusinessUnitItemDataModel, position: Int) {
            listenerBusinessTrackerTracker.onImpressTracking(BusinessUnitTracking.getBusinessUnitView(BusinessUnitTracking.mapToPromotionTracker(element, element.tabName, element.tabPosition+1, positionWidgetOnHome)) as HashMap<String, Any>)
        }
    }
