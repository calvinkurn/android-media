...
class HomeBannerAdapter(itemList: List<CircularModel>, listener: CircularListener, private val cardInteraction: Int) : CircularViewPagerAdapter(itemList, listener) {
...
class HomeBannerImageViewHolder(itemView: View, private val cardInteraction: Int): CircularViewHolder(itemView) {
    companion object{
        private const val FPM_HOMEPAGE_BANNER = "homepage_banner"
    }
    override fun bind(item: CircularModel, listener: CircularListener) {
        itemView.findViewById<CardUnify2>(R.id.banner_card).animateOnPress = cardInteraction
