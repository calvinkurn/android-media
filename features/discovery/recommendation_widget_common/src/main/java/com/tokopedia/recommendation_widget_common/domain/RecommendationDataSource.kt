...
import com.tokopedia.recommendation_widget_common.data.mapper.SingleProductRecommendationMapper.convertIntoRecommendationWidget
...
class RecommendationDataSource(
        ...
        private val singleRecomRawString: String,
    ...
    suspend fun loadSingleWidget(queryParam: String = "", xSource: String = "", pageName: String = "", pageNumber: Int = 0) : RecommendationWidget {
        val data = withContext(Dispatchers.IO){
            val cacheStrategy =
                    GraphqlCacheStrategy.Builder(CacheType.ALWAYS_CLOUD).build()
        ...
        data.getError(SingleProductRecommendationEntity::class.java)?.let {
            if (it.isNotEmpty()) {
                if (!TextUtils.isEmpty(it[0].message)){
                    throw Throwable(it[0].message)
                }
        ...
        return convertIntoRecommendationWidget(data.getSuccessData<SingleProductRecommendationEntity>().productRecommendationWidget?.data)
    }
