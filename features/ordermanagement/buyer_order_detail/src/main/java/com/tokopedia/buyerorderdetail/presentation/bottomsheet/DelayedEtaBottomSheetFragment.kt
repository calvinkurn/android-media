...
class DelayedEtaBottomSheetFragment : BottomSheetUnify() {
    ...
    private fun initView() {
        ...
        if (eta.isNotEmpty()) tvDescription?.text = getString(R.string.delayed_eta_bottomsheet_description, eta)
        setViewListener()
    }

    private fun bindView(view: View) {
        with(view) {
            tvDescription = findViewById(R.id.eta_description)
            btnDismiss = findViewById(R.id.eta_btn_dismiss)
        }
    }
