...
class PackageViewHolder(view: View): AbstractViewHolder<Package>(view) {
    ...
    override fun bind(items: Package) {
        with(itemView){
            ...
            txtPilih_ticket.visibility = if(items.scheduleStatusBahasa == items.isEmpty || items.sold.toInt()>=items.available.toInt()) View.GONE else View.VISIBLE
            txtHabis_ticket.visibility = if(items.scheduleStatusBahasa == items.isEmpty || items.sold.toInt()>=items.available.toInt()) View.VISIBLE else View.GONE
