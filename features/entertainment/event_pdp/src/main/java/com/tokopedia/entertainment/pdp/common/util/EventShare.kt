...
class EventShare (private val activity: Activity) {
    ...
    fun shareTravelAgent(data: ProductDetailData, loadShare: () -> Unit, doneLoadShare: () -> Unit, context: Context) {
        generateBranchLink(data,loadShare,doneLoadShare,context)
    }

    private fun openIntentShare(title: String, context:Context) {
            ...
            LinkerManager.getInstance().executeShareRequest(
                    LinkerUtils.createShareRequest(0,
                        ...
                        override fun urlCreated(linkerShareData: LinkerShareResult) {
                            openIntentShare(data.title, context)
                            doneLoadShare()
                        }
