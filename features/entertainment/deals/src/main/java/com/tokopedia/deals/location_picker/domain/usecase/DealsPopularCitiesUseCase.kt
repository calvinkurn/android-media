...
class DealsPopularCitiesUseCase @Inject constructor(
        @Named(DealsLocationConstants.GQL_POPULAR_CITIES) private val gqlUseCase: GraphqlUseCase<LocationData>
) {
    ...
    private fun generateParams(): Map<String, Any> {
        return mapOf(DealsLocationConstants.REQUEST_PARAM to generateSearchParams())
    }

    private fun generateSearchParams(): ArrayList<RequestParam> {
        val requestParam: ArrayList<RequestParam> = arrayListOf()
        requestParam.add(RequestParam(DealsLocationConstants.MAP_ID, TOP_CITIES))
