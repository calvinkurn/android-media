...
class HeadlineEditKeywordViewModel @Inject constructor(
        ...
        getAdKeywordUseCase.executeQuerySafeMode(
            {
                ...
                onSuccess(getDeepCopyOfSelectedKeywords(),
    ...
    private fun addSelectedKeywords(
        keywordType: String,
        keywords: List<GetKeywordResponse.KeywordsItem>,
    ) {
        if (keywordType == KEYWORD_POSITIVE) {
            val positiveKeywordList =
                keywords.filter { it.type == KEYWORD_TYPE_PHRASE || it.type == KEYWORD_TYPE_EXACT }
            selectedKeywords.addAll(positiveKeywordList)
        } else {
            val positiveKeywordList =
                keywords.filter { it.type == KEYWORD_TYPE_NEGATIVE_PHRASE || it.type == KEYWORD_TYPE_NEGATIVE_EXACT }
            selectedKeywords.addAll(positiveKeywordList)
        }
    }
