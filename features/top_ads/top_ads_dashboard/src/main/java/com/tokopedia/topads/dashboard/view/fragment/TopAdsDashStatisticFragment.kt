...
abstract class TopAdsDashStatisticFragment : TkpdBaseV4Fragment() {
    ...
    private fun getCellData(): List<StatsData> {
        return cells.map {
            when {
                getIndex() == CONST_0 -> StatsData(it.impressionSum.toFloat(),
                    it.impressionSum.toString(),
                    getDate(it))
                getIndex() == CONST_1 -> StatsData(it.clickSum.toFloat(),
                    it.clickSumFmt,
                    getDate(it))
                ...
                getIndex() == CONST_4 -> StatsData(it.ctrPercentage,
                    it.ctrPercentageFmt,
                    getDate(it))
                getIndex() == CONST_5 -> StatsData(it.conversionSum.toFloat(),
                    it.conversionSumFmt,
                    getDate(it))
        ...
        return ChartTooltip(context, TOOLTIP_RES_LAYOUT)
            .setOnDisplayContent { view, data, x, y ->
                (data as? LineChartEntry)?.let {
                    view.findViewById<Typography>(R.id.tooltip_title)?.text = it.xLabel
                    view.findViewById<Typography>(R.id.tooltip_value)?.text = it.yLabel
                }
            }
    }
