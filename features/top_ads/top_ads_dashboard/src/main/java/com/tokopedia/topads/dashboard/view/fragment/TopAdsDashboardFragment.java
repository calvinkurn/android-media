...
public class TopAdsDashboardFragment extends BaseDaggerFragment implements TopAdsDashboardView {
    ...
    @Inject
    TopAdsDashboardTracking tracker;

    @Inject
    TopAdsDashboardRouter router;
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        button.getButton().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                startActivityForResult(router.getTopAdsAddingPromoOptionIntent(getActivity()), REQUEST_CODE_AD_OPTION);
            }
        });
    ...
    private void initStatisticComponent(View view) {
        ...
        viewPager.setOffscreenPageLimit(TopAdsDashboardConstant.OFFSCREEN_PAGE_LIMIT);
        initTabLayouTitles();
    ...
    private void initTopAdsStatisticPagerAdapter() {
        ...
        pagerAdapter = new TopAdsStatisticPagerAdapter(getContext(), getChildFragmentManager(), fragmentList);
    }
    ...
    private void onStoreClicked() {
        ...
        Intent intent = router.getTopAdsDetailShopIntent(getActivity());
        intent.putExtra(TopAdsDashboardConstant.EXTRA_IS_ENOUGH_DEPOSIT, true);
        startActivityForResult(intent, REQUEST_CODE_AD_STATUS);
    }

    private void onSummaryKeywordClicked() {
        tracker.eventTopAdsProductClickKeywordDashboard();
        Intent intent = router.getTopAdsKeywordListIntent(getActivity());

        if (totalGroupAd >= 0) {
            intent.putExtra(TopAdsDashboardConstant.EXTRA_TOTAL_GROUP_ADS, totalGroupAd);
        }
        startActivityForResult(intent, REQUEST_CODE_AD_STATUS);
    }
    ...
    private void onSummaryProductClicked() {
        ...
        startActivityForResult(router.getTopAdsProductAdListIntent(getActivity()), REQUEST_CODE_AD_STATUS);
    }

    private void onSummaryGroupClicked() {
        tracker.eventTopAdsProductClickGroupDashboard();
        Intent intent = router.getTopAdsGroupAdListIntent(getActivity());
        if (totalProductAd >= 0) {
            intent.putExtra(TopAdsDashboardConstant.EXTRA_TOTAL_PRODUCT_ADS, totalProductAd);
        }
        startActivityForResult(intent, REQUEST_CODE_AD_STATUS);
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_AD_STATUS && data != null) {
            ...
            if (data != null){
                int option = data.getIntExtra(TopAdsDashboardConstant.EXTRA_SELECTED_OPTION, -1);
                switch (option){
    ...
    private void checkAdChanged(Intent data){
        ...
        boolean adStatusChanged = data.getBooleanExtra(TopAdsDashboardConstant.EXTRA_AD_CHANGED, false);
        if (adStatusChanged) {
            topAdsDashboardPresenter.clearTotalAdCache();
            loadData();
        }
    ...
    private void gotoCreateProductAd() {
        ...
        this.startActivityForResult(router.getTopAdsGroupNewPromoIntent(getActivity()), REQUEST_CODE_ADD_PRODUCT);
    }

    private void gotoCreateKeyword() {
        topAdsDashboardPresenter.saveSourceTagging(TopAdsSourceOption.SA_MANAGE_KEYWORD_POSITIVE);
        startActivityForResult(router.getTopAdsKeywordNewChooseGroupIntent(getActivity(), true, null),
    ...
    private void trackingDateTopAds(int lastSelection, int selectionType) {
        if(selectionType == DatePickerConstant.SELECTION_TYPE_CUSTOM_DATE){
            ...
            switch (lastSelection){
                case 0:
                    tracker.eventTopAdsShopDatePeriod(TopAdsDashboardTrackerConstant.PERIOD_OPTION_TODAY);
                    break;
                case 1:
                    tracker.eventTopAdsShopDatePeriod(TopAdsDashboardTrackerConstant.PERIOD_OPTION_YESTERDAY);
                    break;
                case 2:
                    tracker.eventTopAdsShopDatePeriod(TopAdsDashboardTrackerConstant.PERIOD_OPTION_LAST_7_DAY);
                    break;
                case 3:
                    tracker.eventTopAdsShopDatePeriod(TopAdsDashboardTrackerConstant.PERIOD_OPTION_LAST_1_MONTH);
                    break;
                case 4:
                    tracker.eventTopAdsShopDatePeriod(TopAdsDashboardTrackerConstant.PERIOD_OPTION_THIS_MONTH);
                    break;
                default:
                    break;
            }
    ...
    private void onCostSelected(){
        switch (selectedStatisticType){
            case TopAdsStatisticsType.PRODUCT_ADS:
                tracker.eventTopAdsProductStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_CPC);
                break;
            case TopAdsStatisticsType.SHOP_ADS:
                tracker.eventTopAdsShopStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_CPC);
                break;
            default:
                break;
        }
    ...
    private void onAverageConversionSelected(){
        switch (selectedStatisticType){
            case TopAdsStatisticsType.PRODUCT_ADS:
                tracker.eventTopAdsProductStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_AVERAGE_CONVERSION);
                break;
            case TopAdsStatisticsType.SHOP_ADS:
                tracker.eventTopAdsShopStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_AVERAGE_CONVERSION);
                break;
            default:
                break;
        }
    ...
    private void onConversionSelected(){
        switch (selectedStatisticType){
            case TopAdsStatisticsType.PRODUCT_ADS:
                tracker.eventTopAdsProductStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_CONVERSION);
                break;
            case TopAdsStatisticsType.SHOP_ADS:
                tracker.eventTopAdsShopStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_CONVERSION);
                break;
            default:
                break;
        }
    ...
    private void onCtrSelected(){
        switch (selectedStatisticType){
            case TopAdsStatisticsType.PRODUCT_ADS:
                tracker.eventTopAdsProductStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_CTR);
                break;
            case TopAdsStatisticsType.SHOP_ADS:
                tracker.eventTopAdsShopStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_CTR);
                break;
            default:
                break;
        }
    ...
    private void onClickSelected(){
        switch (selectedStatisticType){
            case TopAdsStatisticsType.PRODUCT_ADS:
                tracker.eventTopAdsProductStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_CLICK);
                break;
            case TopAdsStatisticsType.SHOP_ADS:
                tracker.eventTopAdsShopStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_CLICK);
                break;
            default:
                break;
        }
    ...
    private void onImpressionSelected(){
        switch (selectedStatisticType){
            case TopAdsStatisticsType.PRODUCT_ADS:
                tracker.eventTopAdsProductStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_IMPRESSION);
                break;
            case TopAdsStatisticsType.SHOP_ADS:
                tracker.eventTopAdsShopStatisticBar(TopAdsDashboardTrackerConstant.STATISTIC_OPTION_IMPRESSION);
                break;
            default:
                break;
        }
