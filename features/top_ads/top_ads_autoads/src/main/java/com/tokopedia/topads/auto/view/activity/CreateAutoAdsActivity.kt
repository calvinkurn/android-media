...
class CreateAutoAdsActivity : BaseSimpleActivity(), HasComponent<AutoAdsComponent> {
    ...
    private fun showAutoAd(){
        updateTitle(getString(topadsautoR.string.auto_ads_creation))
        supportFragmentManager.beginTransaction()
            .replace(parentViewResourceID, CreateAutoAdsFragment.newInstance(), tagFragment)
            .commit()
    }

    private fun showAutoPs(){
        updateTitle(getString(topadsautoR.string.auto_ads_ps_creation))
        supportFragmentManager.beginTransaction()
