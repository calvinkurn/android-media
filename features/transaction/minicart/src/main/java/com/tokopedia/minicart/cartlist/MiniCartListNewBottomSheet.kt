...
class MiniCartListNewBottomSheet : MiniCartListActionListener, MiniCartBottomSheetUnifyListener {
    ...
    private var bottomSheetListener: MiniCartListNewBottomSheetListener? = null
    ...
    @Inject
    internal lateinit var miniCartListDecoration: MiniCartListDecoration

    @Inject
    internal lateinit var summaryTransactionBottomSheet: SummaryTransactionBottomSheet

    @Inject
    internal lateinit var analytics: MiniCartAnalytics

    @Inject
    internal lateinit var miniCartChatListBottomSheet: MiniCartChatListBottomSheet
    ...
    fun show(
        ...
        bottomSheetListener: MiniCartListNewBottomSheetListener
    ) {
        context?.let {
            if (!isShow) {
                this.bottomSheetListener = bottomSheetListener
                val viewBinding =
                    LayoutBottomsheetMiniCartListBinding.inflate(LayoutInflater.from(context))
                ...
                initializeViewModel(viewBinding, fragmentManager, viewModel, lifecycleOwner, viewModelStoreOwner)
                initializeCartData(viewBinding, this.viewModel!!)
            }
        }
    }
    ...
    private fun onResultFromEditBundle(resultCode: Int, data: Intent?) {
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (((oldBundleId.isNotBlank() && newBundleId.isNotBlank() && oldBundleId != newBundleId) || isChangeVariant) && toBeDeletedBundleGroupId.isNotEmpty()) {
                val list =
                    viewModel?.miniCartListBottomSheetUiModel?.value?.visitables ?: emptyList()
                val deletedItems =
                    list.filter { it is MiniCartProductUiModel && it.isBundlingItem && it.bundleId == oldBundleId && it.bundleGroupId == toBeDeletedBundleGroupId }
                toBeDeletedBundleGroupId = ""
                if (deletedItems.isNotEmpty()) {
                    viewModel?.deleteMultipleCartItems(
                        deletedItems as List<MiniCartProductUiModel>,
    ...
    private fun initializeBottomSheet(
        viewBinding: LayoutBottomsheetMiniCartListBinding,
        fragmentManager: FragmentManager
    ) {
        bottomSheet = MiniCartBottomSheetUnify().apply {
            listener = this@MiniCartListNewBottomSheet
            showCloseIcon = false
            showHeader = true
            isDragable = true
            isHideable = true
            ...
            setOnDismissListener {
                ...
                this@MiniCartListNewBottomSheet.viewBinding = null
                this@MiniCartListNewBottomSheet.bottomSheet = null
            }
    ...
    fun showToaster(view: View? = null, message: String, type: Int, ctaText: String = "Oke", isShowCta: Boolean = true, onClickListener: View.OnClickListener? = null) {
        ...
        val toasterViewRoot: View = view ?: return
        toasterViewRoot.let {
    ...
    private fun initializeViewModel(
        ...
        if (viewModel == null) {
            this.viewModel = ViewModelProvider(owner, viewModelFactory)[MiniCartViewModel::class.java]
        } else {
            this.viewModel = viewModel
        }
        this.viewModel?.initializeGlobalState()

        initializeGlobalEventObserver(viewBinding, this.viewModel!!, fragmentManager)
        ...
        observeGlobalEvent(this.viewModel!!, lifecycleOwner)
        observeMiniCartListUiModel(this.viewModel!!, lifecycleOwner)
        observeProductBundleRecomAtcTracker(this.viewModel!!, lifecycleOwner)
    }

    private fun initializeRecyclerView(viewBinding: LayoutBottomsheetMiniCartListBinding) {
        val adapterTypeFactory = MiniCartListAdapterTypeFactory(
            this,
            multiProductBundleCallback(),
            singleProductBundleCallback()
        )
        adapter = MiniCartListAdapter(adapterTypeFactory)
        viewBinding.rvMiniCartList.adapter = adapter
        viewBinding.rvMiniCartList.layoutManager =
            LinearLayoutManager(viewBinding.root.context, LinearLayoutManager.VERTICAL, false)
    ...
    private fun initializeTotalAmount(
        viewBinding: LayoutBottomsheetMiniCartListBinding,
        fragmentManager: FragmentManager,
        context: Context
    ) {
        viewBinding.totalAmount.let {
            miniCartChevronClickListener = View.OnClickListener {
                ...
                viewModel?.miniCartListBottomSheetUiModel?.value?.miniCartSummaryTransactionUiModel?.let { miniCartSummaryTransaction ->
                    summaryTransactionBottomSheet.show(
                        miniCartSummaryTransaction,
                        fragmentManager,
                        context
                    )
                }
    ...
    private fun sendEventClickBuy() {
        ...
        val products =
            viewModel?.miniCartListBottomSheetUiModel?.value?.getMiniCartProductUiModelList()
                ?: emptyList()
        val isOCCFlow = viewModel?.miniCartABTestData?.value?.isOCCFlow ?: false
        analytics.eventClickBuy(pageName, products, isOCCFlow)
    }

    private fun initializeGlobalEventObserver(
        viewBinding: LayoutBottomsheetMiniCartListBinding,
        viewModel: MiniCartViewModel,
        fragmentManager: FragmentManager
    ) {
        globalEventObserver = Observer<GlobalEvent> {
            when (it.state) {
                GlobalEvent.STATE_FAILED_LOAD_MINI_CART_LIST_BOTTOM_SHEET -> {
                    onFailedToLoadMiniCartBottomSheet(it, fragmentManager)
                }
    ...
    private fun onFailedAddToCartProductBundleRecom(
        ...
        viewBinding.bottomsheetContainer.let { container ->
            showToaster(
                view = container,
                message = if (messageError.isNullOrEmpty()) {
    ...
    private fun onFailedUndoDeleteCartItem(
        viewBinding: LayoutBottomsheetMiniCartListBinding,
        globalEvent: GlobalEvent
    ) {
        hideProgressLoading()
    ...
    private fun onSuccessUndoDeleteCartItem(
        ...
        if (message.isNotBlank()) {
            val ctaText = bottomSheet?.context?.getString(R.string.mini_cart_cta_ok)
                ?: ""
            viewBinding.bottomsheetContainer.let { view ->
                showToaster(view, message, Toaster.TYPE_NORMAL, ctaText)
            }
        }
    }
        ...
        bottomSheetUiModelObserver = Observer<MiniCartListUiModel> {
            ...
            viewBinding.totalAmount.totalAmountAdditionalButton.setOnClickListener {
                ...
                miniCartChatListBottomSheet.show(
                    context = viewBinding.totalAmount.context,
                    fragmentManager = fragmentManager,
                    lifecycleOwner = lifecycleOwner,
                    viewModel = viewModel
                )
    ...
    private fun onFailedGoToCheckout(
        viewBinding: LayoutBottomsheetMiniCartListBinding,
        globalEvent: GlobalEvent,
        fragmentManager: FragmentManager
    ) {
        if (globalEvent.observer == GlobalEvent.OBSERVER_MINI_CART_LIST_BOTTOM_SHEET) {
            hideProgressLoading()
            viewBinding.bottomsheetContainer.let { view ->
                handleFailedGoToCheckout(view, view.context, fragmentManager, globalEvent)
            }
        }
    }
    ...
    private fun onFailedDeleteCartItem(
        viewBinding: LayoutBottomsheetMiniCartListBinding,
        globalEvent: GlobalEvent
    ) {
        hideProgressLoading()
    ...
    private fun onFailedToLoadMiniCartBottomSheet(globalEvent: GlobalEvent, fragmentManager: FragmentManager) {
                ...
                bottomSheet?.context?.let {
                    globalErrorBottomSheet.show(
                        ...
                        object : GlobalErrorBottomSheetActionListener {
                            ...
                            override fun onRefreshErrorPage() {
                                showMiniCartListBottomSheet()
                            }
                        }
                    )
                }
            } else {
                showGlobalErrorNoConnection(fragmentManager)
            }
        } else {
            showGlobalErrorNoConnection(fragmentManager)
        }
    ...
    private fun showGlobalErrorNoConnection(fragmentManager: FragmentManager) {
        bottomSheet?.context?.let {
            globalErrorBottomSheet.show(
                fragmentManager,
                it,
    ...
    fun showMiniCartListBottomSheet() {
        bottomSheetListener?.onRetryOpenMiniCartListBottomSheet()
    }

    private fun onSuccessDeleteCartItem(
        viewBinding: LayoutBottomsheetMiniCartListBinding,
        globalEvent: GlobalEvent,
        viewModel: MiniCartViewModel
    ) {
        ...
        if (message.isNotBlank()) {
            ...
            if (data?.isFromEditBundle == true || data?.isBulkDelete == true) {
                viewBinding.bottomsheetContainer.let { view ->
                    showToaster(view, message, Toaster.TYPE_NORMAL)
                }
            } else {
                viewBinding.bottomsheetContainer.let { view ->
                    showToaster(view, message, Toaster.TYPE_NORMAL, ctaText) {
    ...
    private fun observeMiniCartListUiModel(
        viewModel: MiniCartViewModel,
        lifecycleOwner: LifecycleOwner
    ) {
        bottomSheetUiModelObserver?.let {
    ...
    private fun observeProductBundleRecomAtcTracker(
        viewModel: MiniCartViewModel,
        lifecycleOwner: LifecycleOwner
    ) {
        productBundleRecomTrackerObserver?.let {
    ...
    private fun adjustRecyclerViewPadding(viewBinding: LayoutBottomsheetMiniCartListBinding) {
        with(viewBinding) {
            if (rvMiniCartList.canScrollVertically(-1) || rvMiniCartList.canScrollVertically(1) || isBottomSheetFullPage(
                    viewBinding
                )
            ) {
    ...
    private fun isBottomSheetFullPage(viewBinding: LayoutBottomsheetMiniCartListBinding): Boolean {
        ...
        val recyclerViewPaddingBottom =
            viewBinding.rvMiniCartList.resources?.getDimensionPixelSize(R.dimen.dp_64)
                ?: 0
    ...
    private fun setTotalAmountLoading(
        viewBinding: LayoutBottomsheetMiniCartListBinding,
        isLoading: Boolean
    ) {
        with(viewBinding) {
            ...
            if (miniCartWidgetData.totalProductCount == 0) {
                ...
                setAmount(
                    CurrencyFormatUtil.convertPriceValueToIdrFormat(
                        miniCartWidgetData.totalProductPrice,
                        false
                    ).removeDecimalSuffix()
                )
                val ctaText = viewModel?.miniCartABTestData?.value?.buttonBuyWording
    ...
    private fun validateAmountCtaLabel(
        viewBinding: LayoutBottomsheetMiniCartListBinding,
        miniCartWidgetData: MiniCartWidgetData
    ) {
        if (viewModel?.miniCartABTestData?.value?.isOCCFlow == true) {
            viewBinding.totalAmount.post {
                ...
                if (ellipsis > 0) {
                    val ctaText =
                        viewBinding.totalAmount.context.getString(R.string.mini_cart_widget_cta_text_default)
                    if (miniCartWidgetData.totalProductCount == 0) {
    ...
    override fun onDeleteClicked(element: MiniCartProductUiModel) {
        ...
        if (element.isBundlingItem) {
            ...
            val deletedItems =
                list.filter { it is MiniCartProductUiModel && it.isBundlingItem && it.bundleId == element.bundleId && it.bundleGroupId == element.bundleGroupId } as List<MiniCartProductUiModel>
            if (deletedItems.isNotEmpty()) {
    ...
    override fun onBulkDeleteUnavailableItems() {
        ...
        val unavailableProducts =
            viewModel?.miniCartListBottomSheetUiModel?.value?.getUnavailableProduct()
                ?: emptyList()
        ...
        bottomSheet?.context?.let {
            DialogUnify(it, DialogUnify.VERTICAL_ACTION, DialogUnify.NO_IMAGE).apply {
                setTitle(
                    it.getString(
                        R.string.mini_cart_label_dialog_title_delete_unavailable_multiple_item,
                        allUnavailableProducts.size
                    )
                )
    ...
    override fun onShowSimilarProductClicked(appLink: String, element: MiniCartProductUiModel) {
        analytics.eventClickSeeSimilarProductOnUnavailableSection(
            element.productId,
            element.errorType
        )
        bottomSheet?.context?.let {
