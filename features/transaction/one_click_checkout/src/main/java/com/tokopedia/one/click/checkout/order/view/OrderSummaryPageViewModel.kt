...
import com.tokopedia.one.click.checkout.order.analytics.OrderSummaryPageEnhanceECommerce
import com.tokopedia.one.click.checkout.order.data.UpdateCartOccCartRequest
import com.tokopedia.one.click.checkout.order.data.UpdateCartOccProfileRequest
import com.tokopedia.one.click.checkout.order.data.UpdateCartOccRequest
import com.tokopedia.one.click.checkout.order.data.checkout.*
import com.tokopedia.one.click.checkout.order.domain.CheckoutOccUseCase
import com.tokopedia.one.click.checkout.order.domain.GetOccCartUseCase
...
class OrderSummaryPageViewModel @Inject constructor(dispatcher: CoroutineDispatcher,
    ...
    private fun generateShopPromos(): List<com.tokopedia.one.click.checkout.order.data.checkout.PromoRequest> {
        val finalPromo = validateUsePromoRevampUiModel
        if (finalPromo != null) {
            val list: ArrayList<com.tokopedia.one.click.checkout.order.data.checkout.PromoRequest> = ArrayList()
    ...
    private fun generateCheckoutPromos(): List<com.tokopedia.one.click.checkout.order.data.checkout.PromoRequest> {
        val list = ArrayList<com.tokopedia.one.click.checkout.order.data.checkout.PromoRequest>()
        val finalPromo = validateUsePromoRevampUiModel
    ...
    private fun generateOspEe(step: Int, option: String, promoCodes: List<String> = emptyList()): Map<String, Any> {
        return OrderSummaryPageEnhanceECommerce().apply {
            ...
            setBrand(null)
            setCategory(orderProduct.productResponse.category)
            setVariant(null)
            ...
            setWarehouseId(orderProduct.productResponse.wareHouseId.toString())
            setProductWeight(orderProduct.weight.toString())
            setPromoCode(promoCodes)
            setPromoDetails("")
            setProductType("")
            setCartId(orderShop.cartResponse.cartId.toString())
            ...
            setCategoryId(orderProduct.productResponse.categoryId.toString())
        }.build(step, option)
    }
