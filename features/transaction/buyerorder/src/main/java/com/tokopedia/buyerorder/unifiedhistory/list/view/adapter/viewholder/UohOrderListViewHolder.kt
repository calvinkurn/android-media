 ...
 */
class UohOrderListViewHolder(itemView: View, private val isLoading: Boolean) : UohItemAdapter.BaseViewHolder<UohTypeData>(itemView) {
            ...
            if (isLoading) {
                ...
                if (item.dataObject.metadata.dotMenus.isNotEmpty()) {
                    itemView.iv_kebab_menu?.setOnClickListener {
                        // actionListener?.onKebabMenuClicked(item.dataObject.data.uohOrders.orders[position].metadata.dotMenus)
                    }
                }
                ...
                itemView.cl_data_product?.setOnClickListener {
                    // actionListener?.onListItemClicked(item.dataObject.data.uohOrders.orders[position].verticalCategory, item.dataObject.data.uohOrders.orders[position].verticalID, item.dataObject.data.uohOrders.orders[position].metadata.upstream)
                }

                itemView.uoh_btn_action?.setOnClickListener {
                    if (item.dataObject.metadata.buttons.isNotEmpty()) {
                        // actionListener?.onActionButtonClicked(item.dataObject.data.uohOrders.orders[position].metadata.buttons.first().appURL)
                    }
                }
            }
        }
