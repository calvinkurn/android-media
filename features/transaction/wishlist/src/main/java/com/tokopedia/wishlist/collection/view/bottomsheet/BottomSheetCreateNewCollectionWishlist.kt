...
import com.tokopedia.wishlist.collection.view.fragment.IWishlistCollectionFragment
...
class BottomSheetCreateNewCollectionWishlist :
    ...
    private fun initLayout() {
        ...
        setTitle(getString(wishlistR.string.collection_create_bottomsheet_title))
    }
    ...
    private fun disableSaveButton() {
        binding?.run {
            collectionCreateButton.apply {
                text = getText(wishlistR.string.collection_create_bottomsheet_button_label)
                isEnabled = false
                setOnClickListener { }
            }
        }
    ...
    private fun convertToSaveButton() {
        if (_productIds.isNotEmpty()) {
            binding?.run {
                collectionCreateButton.apply {
                    isEnabled = true
                    text = getString(wishlistR.string.collection_save_to_existing_collection)
                    setOnClickListener {
    ...
    private fun convertToCreateButton() {
        if (_productIds.isNotEmpty()) {
            binding?.run {
                collectionCreateButton.apply {
                    isEnabled = true
                    text = getString(wishlistR.string.collection_create_bottomsheet_button_label)
                    setOnClickListener {
                ...
                is Success -> {
                    if (result.data.status == OK) {
                        ...
                        val errorMessage = result.data.errorMessage.first().ifEmpty {
                            getString(
                                wishlistR.string.wishlist_common_error_msg
                            )
                        }
            ...
            when (result) {
                is Success -> {
                    if (result.data.status == OK && result.data.dataCreate.success) {
                        ...
                        val errorMessage = result.data.errorMessage.first().ifEmpty {
                            getString(wishlistR.string.wishlist_common_error_msg)
                        }
