...
class GetWishlistV2UseCase @Inject constructor(
    ...
    override suspend fun executeOnBackground(): Result<GetWishlistV2Response.Data.WishlistV2> {
        return try {
            val request =
                GraphqlRequest(GetWishlistV2(), GetWishlistV2Response.Data::class.java, params)
            val response = graphqlRepository.response(listOf(request))
                .getSuccessData<GetWishlistV2Response.Data>()
    ...
    fun setParams(param: WishlistV2Params) {
        param.wishlistChosenAddress = chosenAddressRequestHelper.getChosenAddress()
