...
class EnhancedECommerceProductCartMapData {
    ...
    fun setAttribution(data: String) {
        Product[KEY_ATTRIBUTION] = if (data.isNotBlank()) data else DEFAULT_VALUE_NONE_OTHER
    }

    fun setDimension38(data: String) {
        Product[KEY_DIMENSION_38] = if (data.isNotBlank()) data else DEFAULT_VALUE_NONE_OTHER
    }
    ...
    fun setDimension80(data: String) {
        Product[KEY_DIMENSION_80] = if (data.isNotBlank()) data else DEFAULT_VALUE_NONE_OTHER
    }

    fun setListName(data: String) {
        Product[KEY_LIST] = if (data.isNotBlank()) data else DEFAULT_VALUE_NONE_OTHER
    }
    ...
    fun setPosition(position: String) {
        Product[KEY_POS] = if (position.isNotBlank()) position else DEFAULT_VALUE_NONE_OTHER
    }
    ...
    fun setCartId(cartId: String) {
        Product[KEY_CART_ID] = if (cartId.isNotBlank()) cartId else DEFAULT_VALUE_NONE_OTHER
    }
    ...
    fun setDimension12(shippingCourierPrice: String) {
        Product[KEY_DIMENSION_12] = if (shippingCourierPrice.isNotBlank()) shippingCourierPrice else DEFAULT_VALUE_NONE_OTHER
    }

    fun setWarehouseId(warehouseId: String) {
        Product[KEY_WAREHOUSE_ID] = if (warehouseId.isNotBlank()) warehouseId else DEFAULT_VALUE_NONE_OTHER
    }

    fun setProductWeight(productWeight: String) {
        Product[KEY_PRODUCT_WEIGHT] = if (productWeight.isNotBlank()) productWeight else DEFAULT_VALUE_NONE_OTHER
    }

    fun setPromoCode(promoCodes: String) {
        Product[KEY_PROMO_CODE] = if (promoCodes.isNotBlank()) promoCodes else ""
    }

    fun setPromoDetails(promoDetails: String) {
        Product[KEY_PROMO_DETAILS] = if (promoDetails.isNotBlank()) promoDetails else ""
    }

    fun setBuyerAddressId(buyerAddressId: String) {
        Product[KEY_BUYER_ADDRESS_ID] = if (buyerAddressId.isNotBlank()) buyerAddressId else ""
    }

    fun setShippingDuration(shippingDuration: String) {
        Product[KEY_SHIPPING_DURATION] = if (shippingDuration.isNotBlank()) shippingDuration else ""
    }

    fun setCourier(courier: String) {
        Product[KEY_COURIER] = if (courier.isNotBlank()) courier else ""
    }

    fun setShippingPrice(shippingPrice: String) {
        Product[KEY_SHIPPING_PRICE] = if (shippingPrice.isNotBlank()) shippingPrice else ""
    }

    fun setCodFlag(codFlag: String) {
        Product[KEY_COD_FLAG] = if (codFlag.isNotBlank()) codFlag else ""
    }

    fun setTokopediaCornerFlag(tokopediaCornerFlag: String) {
        Product[KEY_TOKOPEDIA_CORNER_FLAG] = if (tokopediaCornerFlag.isNotBlank()) tokopediaCornerFlag else ""
    }

    fun setIsFulfillment(isFulfillment: String) {
        Product[KEY_IS_FULFILLMENT] = if (isFulfillment.isNotBlank()) isFulfillment else ""
    }

    fun setPicture(picture: String) {
        Product[KEY_PICTURE] = if (picture.isNotBlank()) picture else ""
    }

    fun setUrl(url: String) {
        Product[KEY_URL] = if (url.isNotBlank()) url else ""
    }

    fun setDimension52(shopId: String) {
        Product[KEY_DIMENSION_52] = if (shopId.isNotBlank()) shopId else ""
    }
    ...
    fun setDimension57(shopName: String) {
        Product[KEY_DIMENSION_57] = if (shopName.isNotBlank()) shopName else ""
    }

    fun setDimension59(shopType: String) {
        Product[KEY_DIMENSION_59] = if (shopType.isNotBlank()) shopType else ""
    }

    fun setDimension77(cartId: String) {
        Product[KEY_DIMENSION_77] = if (cartId.isNotBlank()) cartId else ""
    }
