...
class SomBottomSheetBuyerNoResponse(
...
): SomBaseRejectOrderBottomSheet<BottomsheetSecondaryBinding>(context, LAYOUT, SomConsts.VALUE_REASON_BUYER_NO_RESPONSE) {
    ...
    override fun setupChildView() {
        binding?.run {
            ...
            tfExtraNotes.textFiedlLabelText.text = context.getString(R.string.buyer_no_resp_label)
            tfExtraNotes.setPlaceholder(context.getString(R.string.buyer_no_resp_placeholder))
