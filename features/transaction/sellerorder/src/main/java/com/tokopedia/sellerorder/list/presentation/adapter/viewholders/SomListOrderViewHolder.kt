...
class SomListOrderViewHolder(
    ...
    private fun onQuickActionButtonClicked(element: SomListOrderUiModel) {
        element.buttons.firstOrNull()?.let { button ->
            when (button.key) {
                ...
                KEY_CONFIRM_SHIPPING -> listener.onConfirmShippingButtonClicked(button.displayName, element.orderId)
                KEY_ACCEPT_ORDER -> listener.onAcceptOrderButtonClicked(button.displayName, element.orderId)
                KEY_REQUEST_PICKUP -> listener.onRequestPickupButtonClicked(button.displayName, element.orderId)
    ...
    interface SomListOrderItemListener {
        ...
        fun onConfirmShippingButtonClicked(actionName: String, orderId: String)
        fun onAcceptOrderButtonClicked(actionName: String, orderId: String)
        fun onRequestPickupButtonClicked(actionName: String, orderId: String)
