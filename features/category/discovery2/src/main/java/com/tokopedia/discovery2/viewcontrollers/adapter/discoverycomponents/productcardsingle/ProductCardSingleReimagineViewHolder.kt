...
class ProductCardSingleReimagineViewHolder(itemView: View, val fragment: Fragment) :
    ...
    private var productViewHolder: AbstractViewHolder =
        DiscoveryHomeFactory.createViewHolder(
            View.inflate(
                fragment.context,
                ComponentsList.ProductCardSingleItemReimagine.id,
                masterProdFrameLayout
            ),
            ComponentsList.ProductCardSingleItemReimagine.ordinal,
            fragment
    ...
    private fun handleProductData(productData: ComponentsItem?) {
        if (productData == null) {
            ...
            if (fragment.context?.applicationContext as? Application != null) {
                ...
                shimmer.hide()
                productViewModel =
                    DiscoveryHomeFactory.createViewModel(ComponentsList.ProductCardSingleItemReimagine.ordinal)(
