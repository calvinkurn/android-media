...
class LihatSemuaViewHolder(itemView: View, private val fragment: Fragment) : AbstractViewHolder(itemView, fragment.viewLifecycleOwner) {
    ...
    private var lihatSemuaViewModel: LihatSemuaViewModel? = null
    ...
    override fun setUpObservers(lifecycleOwner: LifecycleOwner?) {
        ...
        lifecycleOwner?.let {
            lihatSemuaViewModel?.getComponentData()?.observe(
                it,
            ...
            lihatSemuaViewModel?.getRestartTimerAction()?.observe(it) { shouldStartTimer ->
                if (shouldStartTimer && (lihatSemuaViewModel?.getStartDate()?.isNotEmpty() == true || lihatSemuaViewModel?.getEndDate()?.isNotEmpty() == true)) {
    ...
    private fun cornersForBackground(view: View) {
        try {
