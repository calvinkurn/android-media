...
class TimerSprintSaleItemViewHolder(itemView: View, private val fragment: Fragment) : AbstractViewHolder(itemView, fragment.viewLifecycleOwner) {
    private var timerSprintSaleItemViewModel: TimerSprintSaleItemViewModel? = null
    ...
    override fun setUpObservers(lifecycleOwner: LifecycleOwner?) {
        ...
        lifecycleOwner?.let {
            timerSprintSaleItemViewModel?.getComponentLiveData()?.observe(
                it,
            ...
            timerSprintSaleItemViewModel?.refreshPage()?.observe(
                it,
                Observer { refreshPage ->
                    if (refreshPage) (fragment as DiscoveryFragment).onRefresh()
                }
            )
            timerSprintSaleItemViewModel?.getSyncPageLiveData()?.observe(
                it,
                Observer { needResync ->
    ...
    private fun setTimerType() {
        timerSprintSaleItemViewModel?.let { timerSprintSaleItemViewModel ->
