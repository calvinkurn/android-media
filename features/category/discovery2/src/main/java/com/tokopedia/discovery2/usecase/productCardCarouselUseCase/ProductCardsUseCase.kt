...
class ProductCardsUseCase @Inject constructor(private val productCardsRepository: ProductCardsRepository) {
    ...
    suspend fun loadFirstPageComponents(componentId: String, pageEndPoint: String, queryMap: Map<String, String?>?): Boolean {
        ...
        component?.let { item ->
            ...
            if (productListData.isEmpty()) return false
    ...
    suspend fun getCarouselPaginatedData(componentId: String, pageEndPoint: String, rpcDiscoQuery: Map<String, String?>?): Boolean {
        val component = getComponent(componentId, pageEndPoint)
        component?.let { item ->
            val parentComponentsItem = getComponent(item.parentComponentId, pageEndPoint)
            val isDynamic = item.properties?.dynamic ?: false
            ...
            val productListData = productCardsRepository.getProducts(
                    ...
                    item.name)
            if(productListData.isEmpty()) return false
            (item.getComponentsItem() as ArrayList<ComponentsItem>).addAll(productListData)
            return true
        }
        return false
    }
