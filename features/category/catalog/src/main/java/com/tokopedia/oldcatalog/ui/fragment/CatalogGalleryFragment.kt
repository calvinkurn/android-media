...
import com.tokopedia.oldcatalog.adapter.gallery.CatalogBottomGalleyRecyclerViewAdapter
import com.tokopedia.oldcatalog.adapter.gallery.CatalogGalleryImagePagerAdapter
...
class CatalogGalleryFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (catalogImages != null) {
            ...
            view_pager_intermediary.addOnPageChangeListener(object : ViewPager.OnPageChangeListener{
                ...
                override fun onPageSelected(position: Int) {
                    ...
                    if(showBottomNumbering){
                        numbering.text = (context?.resources?.getString(R.string.catalog_numbering_images, (position + 1).toString(), catalogImages?.size.toString()) ?: "")
                    }
                }
            ...
            if(showBottomNumbering){
                numbering.text = (context?.resources?.getString(R.string.catalog_numbering_images, (currentImage + 1).toString(), catalogImages?.size.toString()) ?: "")
                numbering.show()
            }
        }
