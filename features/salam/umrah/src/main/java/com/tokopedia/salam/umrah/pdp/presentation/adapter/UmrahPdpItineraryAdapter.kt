...
class UmrahPdpItineraryAdapter : RecyclerView.Adapter<UmrahPdpItineraryAdapter.UmrahPdpItineraryViewHolder>(){
    private val _item: MutableList<UmrahPdpItineraryItem> = mutableListOf()
    ...
    fun updateItems(items: List<UmrahPdpItineraryItem>){
        _item.clear()
        _item.addAll(items)
    ...
    inner class UmrahPdpItineraryViewHolder(view: View) : RecyclerView.ViewHolder(view){
        fun bindItem(item: UmrahPdpItineraryItem) {
            with(itemView){
    ...
    data class UmrahPdpItineraryItem (
            val day: Int = 0,
            val desc: String = ""
    )
}