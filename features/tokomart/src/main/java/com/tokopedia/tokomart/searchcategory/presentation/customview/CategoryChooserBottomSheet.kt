...
class CategoryChooserBottomSheet: BottomSheetUnify(), OptionRadioListener {
    ...
    private fun configureViews() {
        ...
        adapter = CategoryChooserAdapter(filter.options, this, selectedOption)
        recyclerView?.let {
            it.adapter = adapter
    ...
    override fun onChecked(position: Int, option: Option, isChecked: Boolean) {
        ...
        notifyAdapterSelection(position, option)
        configureButtonApplyVisibility(position)

        callback?.getResultCount(option)
    }

    private fun notifyAdapterSelection(position: Int, option: Option) {
        val prevPosition = selectedOption?.index ?: 0
    ...
    private class CategoryChooserAdapter(
            ...
            private val checkedOption: IndexedValue<Option>,
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CategoryChooserViewHolder {
            ...
            return CategoryChooserViewHolder(view, optionRadioListener, checkedOption)
        }
    ...
    private class CategoryChooserViewHolder(
        ...
        fun bind(item: Option) {
            ...
            radio?.isChecked = adapterPosition == checkedOption.index
