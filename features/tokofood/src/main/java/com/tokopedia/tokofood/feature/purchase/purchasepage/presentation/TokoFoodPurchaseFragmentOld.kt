...
open class TokoFoodPurchaseFragmentOld :
    ...
    private val viewModel by lazy {
        ViewModelProvider(this, viewModelFactory).get(TokoFoodPurchaseViewModelOld::class.java)
    }
    ...
    private fun initializeToolbar() {
        activity?.let {
            ...
            toolbar?.let { toolbar ->
                toolbar.listener = this@TokoFoodPurchaseFragmentOld
            ...
            activityViewModel?.cartDataValidationFlow?.collect {
                ...
                when (it.state) {
                    ...
                    UiEvent.EVENT_SUCCESS_UPDATE_NOTES -> {
                        if (it.source == SOURCE) {
                            it.data?.getSuccessUpdateResultPairOld()?.let { (_, cartTokoFoodData) ->
                    ...
                    UiEvent.EVENT_SUCCESS_UPDATE_QUANTITY -> {
                        if (it.source == SOURCE) {
                            it.data?.getSuccessUpdateResultPairOld()?.let { (updateParams, cartTokoFoodData) ->
                                viewBinding?.recyclerViewPurchase?.post {
        ...
        viewLifecycleOwner.lifecycleScope.launchWhenStarted {
            viewModel.trackerLoadCheckoutData
                .collect { checkoutData ->
                    ...
                    TokoFoodPurchaseAnalyticsOld.sendLoadCheckoutTracking(checkoutData, userId)
                }
        }
    }
    ...
    private fun collectTrackerPaymentCheckoutData() {
        viewLifecycleOwner.lifecycleScope.launchWhenStarted {
            viewModel.trackerPaymentCheckoutData
                .collect { checkoutData ->
                    val userId = userSession.userId
                    TokoFoodPurchaseAnalyticsOld.sendSuccessChoosePayment(checkoutData, userId)
                }
        }
    }
        ...
        if (data.isShowBottomsheet) {
            consentBottomSheet = TokoFoodPurchaseConsentBottomSheet.createInstance(
                ...
                setOnDismissListener {
                    this@TokoFoodPurchaseFragmentOld.viewModel.setPaymentButtonLoading(false)
                }
            }
    ...
    override fun onPromoWidgetClicked() {
        navigateToNewFragment(TokoFoodPromoFragmentOld.createInstance(SOURCE))
    }
        ...
        val addOnBottomSheet = TokoFoodPurchaseNoteBottomSheet(
            ...
            object : TokoFoodPurchaseNoteBottomSheet.Listener {
                override fun onSaveNotesClicked(notes: String) {
                    val updateParam =
                        TokoFoodPurchaseUiModelMapperOld.mapUiModelToUpdateParam(
