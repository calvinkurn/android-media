...
import com.tokopedia.tokofood.databinding.FragmentMerchantPageLayoutOldBinding
import com.tokopedia.tokofood.feature.merchant.analytics.MerchantPageAnalyticsOld
...
import com.tokopedia.tokofood.feature.merchant.presentation.adapter.ProductListAdapterOld
...
class MerchantPageFragmentOld : BaseMultiFragment(),
    ...
    private var binding: FragmentMerchantPageLayoutOldBinding? = null
    ...
    @Inject
    lateinit var merchantPageAnalytics: MerchantPageAnalyticsOld
    ...
    private val viewModel by lazy {
        viewModelProvider.get(MerchantPageViewModelOld::class.java)
    }
    ...
    private var productListAdapter: ProductListAdapterOld? = null
    ...
    override fun onCreateView(
        ...
        val viewBinding = FragmentMerchantPageLayoutOldBinding.inflate(inflater)
        binding = viewBinding
    ...
    private fun setupCardSticky() {
        binding?.cardUnifySticky?.run {
            setOnClickListener {
                ...
                bottomSheet.setCategoryFilterListener(this@MerchantPageFragmentOld)
    ...
    private fun showMerchantShareBottomSheet(
        ...
        if (isAdded) {
            ...
            universalShareBottomSheet = UniversalShareBottomSheet.createInstance().apply {
                init(this@MerchantPageFragmentOld)
                setUtmCampaignData(
        ...
        activityViewModel?.cartDataValidationFlow?.collect {
            when (it.state) {
                ...
                UiEvent.EVENT_SUCCESS_ADD_TO_CART -> {
                    if (it.source == SOURCE) {
                        onSuccessAddCart(it.data?.getSuccessUpdateResultPairOld())
                    }
                }
                    ...
                    if (it.source == SOURCE) {
                        ...
                        bottomSheetData?.run {
                            if (isShowBottomSheet) {
                                ...
                                bottomSheet.setClickListener(this@MerchantPageFragmentOld)
                                bottomSheet.show(childFragmentManager)
                            }
                        }
                    }
                ...
                UiEvent.EVENT_SUCCESS_UPDATE_CART -> {
                    if (it.source == SOURCE) {
                        onSuccessUpdateCart(it.data?.getSuccessUpdateResultPairOld())
                    }
                }
    ...
    private fun setupProductList() {
        productListAdapter = ProductListAdapterOld(this, this)
    ...
    override fun onProductCardClicked(productListItem: ProductListItem, cardPositions: Pair<Int, Int>) {
        ...
        bottomSheet.setListener(this@MerchantPageFragmentOld)
