...
//    }
    ...
    private fun setData() {
        ...
        proceedButton.text = activationBottomSheetDetail?.widgetBottomSheet?.buttons?.get(0)?.buttonText
    ...
    private fun setHeaderIcon() {
        if (context?.isDarkMode() == true)
            activationBottomSheetDetail?.widgetBottomSheet?.productIconDark?.let { headerIcon.setImageUrl(it) }
        else
            activationBottomSheetDetail?.widgetBottomSheet?.productIconLight?.let { headerIcon.setImageUrl(it) }
