package com.tokopedia.common_digital.common.constant

object DigitalExtraParam {

    @JvmField
    val EXTRA_PASS_DIGITAL_CART_DATA = "EXTRA_PASS_DIGITAL_CART_DATA"

    @JvmField
    val EXTRA_PASS_DIGITAL_SUBSCRIPTION_DATA = "EXTRA_PASS_DIGITAL_SUBSCRIPTION_DATA"

    @JvmField
    val EXTRA_MESSAGE = "EXTRA_MESSAGE"

    @JvmField
    val EXTRA_PARAM_TELCO = "EXTRA_PARAM_TELCO"

    @JvmField
    val EXTRA_PARAM_VOUCHER_GAME = "EXTRA_PARAM_VOUCHER_GAME"

    @JvmField
    val EXTRA_CATEGORY_PASS_DATA = "EXTRA_CATEGORY_PASS_DATA"

    @JvmField
    val EXTRA_NFC_FROM_PDP = "calling_from_pdp"

    @JvmField
    val EXTRA_NFC = "calling_from_nfc"
}