...
public class CategoryDetailDataSource {
    ...
    public Observable<ProductDigitalData> getCategoryDetailWithFavorit(String categoryId) {
        return Observable.concat(getCategoryAndFavDataFromLocal(categoryId),
                getCategoryAndFavoritFromCloud(categoryId, "", "", ""))
    ...
    public Observable<ProductDigitalData> getCategoryAndFavoritFromCloud(String categoryId,
                                                                         String operatorId, String clientNumber, String productId) {
        return digitalGqlApi.getCategoryAndFavoriteList(getCategoryAndFavRequestPayload(categoryId,
                operatorId, clientNumber, productId))
                .map(response -> {
                    ...
                    if(response != null && response.body() != null && response.body().size() > 1) {
                        ...
                        newMappedObject.setRechargeCategoryDetail(
                                responseEntity.get(0).getData().getRechargeCategoryDetail());
                        newMappedObject.setRechargeFavoritNumberResponseEntity(
                                responseEntity.get(1).getData().getRechargeFavoritNumberResponseEntity());
                    }
    ...
    private Action1<RechargeResponseEntity> saveCategoryDetailToCache(final String categoryId) {
        return digitalCategoryDetailEntity -> {
            cacheManager.save(DigitalCache.NEW_DIGITAL_CATEGORY_DETAIL + "/" + categoryId,
    ...
    private Action1<RechargeResponseEntity> saveCategoryDetailAndFavToCache(final String categoryId) {
        return digitalCategoryDetailEntity -> {
            cacheManager.save(DigitalCache.NEW_DIGITAL_CATEGORY_AND_FAV + "/" + categoryId,
                    CacheUtil.convertModelToString(digitalCategoryDetailEntity,
                            new TypeToken<RechargeResponseEntity>() {
                            }.getType()),
                    900);
