 ...
 */
class FlightAirportListDbSource @Inject
constructor(private val flightAirportCountryDao: FlightAirportCountryDao,
    ...
    val data: List<FlightAirportCountry>
        get() {
            val g = Gson()
            val dataResponseType = object : TypeToken<DataResponse<List<FlightAirportCountry>>>() {

            }.type
            val dataResponse = g.fromJson<DataResponse<List<FlightAirportCountry>>>(loadJSONFromAsset(), dataResponseType)
                ...
                .flatMap { it ->
                    if (it.isEmpty()) {
                        return@flatMap Observable.just(data)
                                .map<List<FlightAirportCountryTable>> { flightCountryData -> flightAirportMapper.mapEntitiesToTables(flightCountryData) }
