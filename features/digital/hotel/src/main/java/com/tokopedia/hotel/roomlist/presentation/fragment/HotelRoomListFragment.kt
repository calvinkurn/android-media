...
class HotelRoomListFragment : BaseListFragment<HotelRoom, RoomListTypeFactory>(), ChipAdapter.OnClickListener,
    ...
    override fun onItemClicked(room: HotelRoom) {
        trackingHotelUtil.hotelClickRoomDetails(hotelRoomListPageModel.propertyId, room.roomId, ceil(room.roomPrice.priceAmount).toString())
    ...
    override fun onPhotoClickListener(room: HotelRoom) {
        trackingHotelUtil.hotelClickRoomListPhoto(room.additionalPropertyInfo.propertyId, room.roomId, ceil(room.roomPrice.priceAmount).toString())
    }
