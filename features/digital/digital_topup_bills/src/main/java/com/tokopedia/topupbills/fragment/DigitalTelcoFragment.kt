 ...
 */
class BaseDigitalTelcoFragment: BaseDaggerFragment() {

    private lateinit var headerView: DigitalProductHeaderView

    override fun getScreenName(): String {
        return BaseDigitalTelcoFragment::class.java.simpleName
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.fragment_digital_base_telco, container, false)
        ...
        headerView.setListener(object: DigitalProductHeaderView.ActionListener {
            override fun onClickSubMenu(subMenu: DigitalProductSubMenu) {
                if (subMenu.name.equals("telco-prepaid")) {
                    replaceFragment(DigitalTelcoPraFragment.newInstance())
                } else {
                    Toast.makeText(context, subMenu.label, Toast.LENGTH_SHORT).show()
//                  replaceFragment(DigitalTelcoPostFragment.newInstance())
                }
            }
        })
