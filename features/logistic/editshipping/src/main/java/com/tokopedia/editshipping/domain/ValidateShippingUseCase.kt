...
class ValidateShippingUseCase(
    val context: Context,
    val useCase: GraphqlUseCase,
    val mapper: ValidateShippingMapper
) : UseCase<ValidateShippingModel>() {
    ...
    override fun createObservable(requestParams: RequestParams): Observable<ValidateShippingModel> {
        ...
        return useCase.createObservable(RequestParams.EMPTY)
            .map {
                val response =
                    it.getData<ValidateShippingResponse>(ValidateShippingResponse::class.java)
                if (response != null) {
