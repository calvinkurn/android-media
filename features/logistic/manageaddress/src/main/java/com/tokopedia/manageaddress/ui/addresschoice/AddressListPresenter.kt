...
class AddressListPresenter @Inject constructor(
    ...
    override fun getAddress(prevState: Int, localChosenAddrId: Int) {
        usecase.execute(EMPTY_STRING, prevState, localChosenAddrId)
    ...
    override fun searchAddress(query: String, prevState: Int, localChosenAddrId: Int) {
         usecase.execute(query, prevState, localChosenAddrId)
                .doOnSubscribe { mView?.showLoading() }
                .doOnTerminate {
                    mView?.hideLoading()
                    mView?.stopTrace()
    ...
    override fun loadMore(prevState: Int, localChosenAddrId: Int) {
        // Always true until has_next property is ready from backend, still works fine
        // if (!mHasNext) return
        usecase.loadMore(mCurrentQuery, mCurrentPage + 1, prevState, localChosenAddrId)
                .doOnSubscribe { mView?.showLoading() }
                .doOnTerminate {
                    mView?.hideLoading()
                    mView?.stopTrace()
