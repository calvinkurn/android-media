...
class FlightPassengerViewModel @Inject constructor(
    ...
    fun getContactList(query: GqlQueryInterface, filterType: String = "") {
        launch {
            val contacts = getContactListUseCase.execute(
                query = query,
    ...
    fun getNationalityById(rawQuery: GqlQueryInterface, paramId: String) {
        launch(dispatcherProvider.main) {
            val result = getPhoneCodeByIdUseCase.execute(rawQuery, paramId)
