...
class HotelHomepageViewModelTest {
    ...
    fun setUp() {
        ...
        hotelHomepageViewModel = HotelHomepageViewModel(
            this.graphqlRepository,
            getTravelCollectiveBannerUseCase,
            travelRecentSearchUseCase,
            getPropertyPopularUseCase,
            getDppoConsentUseCase,
            travelTickerCoroutineUseCase,
            dispatcher
        )
    }
    ...
    fun getDefaultHomeParameter_shouldReturnData() {
        ...
        coEvery { graphqlRepository.response(any(), any()) } returns
            GraphqlResponse(
                mapOf<Type, Any>(HotelPropertyDefaultHome.Response::class.java to data),
    ...
    fun deleteRecentSearch_shouldReturnSuccess() {
        ...
        coEvery { graphqlRepository.response(any(), any()) } returns
            GraphqlResponse(
                mapOf<Type, Any>(HotelDeleteRecentSearchEntity.Response::class.java to HotelDeleteRecentSearchEntity.Response(HotelDeleteRecentSearchEntity(true))),
                mapOf<Type, List<GraphqlError>>(),
                false
            )
    ...
    fun deleteRecentSearch_shouldReturnFail() {
        ...
        coEvery { graphqlRepository.response(any(), any()) } returns
            GraphqlResponse(
                mapOf<Type, Any>(),
                mapOf<Type, List<GraphqlError>>(GraphqlError::class.java to listOf(GraphqlError())),
                false
            )
    ...
    fun getTickerData_shouldData() {
        ...
        val response = TravelTickerModel(
            title = title, message = message, url = "", type = 0, status = 0,
    ...
    fun getDppoConsentRecharge_Success() {
        ...
        // when
        hotelHomepageViewModel.getDppoConsent()

        // then
        val actualData = hotelHomepageViewModel.dppoConsent.value
