package com.scp.auth.registerpushnotif.services
...
class RegisterPushNotificationWorker(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (keyPair != null) {
                signData(userSession.userId.orEmpty(), userSession.deviceId.orEmpty()).let {
                    try {
                        return registerPushNotificationUseCase(
                            RegisterPushNotificationParamsModel(
    ...
    companion object {
        ...
        private fun recordLog(type: String, message: String, throwable: Throwable) {
            ServerLogger.log(
                Priority.P2, TAG_REGISTER_PUSH_NOTIF,
                mapOf(
                    "type" to type,
                    "message" to message,
