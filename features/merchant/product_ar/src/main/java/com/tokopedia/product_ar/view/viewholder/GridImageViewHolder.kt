...
class GridImageViewHolder(itemView: View, val listener: asdInterface) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(image: Bitmap) {
        ...
        if (listener.getRvHeight() != 0) {
            imgFull?.layoutParams?.height = listener.getRvHeight() / 2 -
