...
class GetTopAdsShopUseCaseWithCoroutine(private val favoriteRepository: FavoriteRepository): UseCase<TopAdsShop>() {
    ...
    override suspend fun executeOnBackground(): TopAdsShop {
        val isFreshData = isForceRefresh(requestParams)
        val param = requestParams.parameters
        return if (isFreshData) {
            favoriteRepository.suspendGetFreshTopAdsShop(requestParams.parameters)
        } else {
            favoriteRepository.suspendGetTopAdsShop(param)
        }
    }
