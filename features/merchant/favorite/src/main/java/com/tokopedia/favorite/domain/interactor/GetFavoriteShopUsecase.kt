...
class GetFavoriteShopUsecase(
    ...
    override fun createObservable(requestParams: RequestParams): Observable<FavoriteShop> {
        val isFirstPage = requestParams.getBoolean(KEY_IS_FIRST_PAGE, false)
        ...
        return if (isFirstPage) {
            favoriteRepository.getFirstPageFavoriteShop(paramsAllInString)
        } else {
            favoriteRepository.getFavoriteShop(paramsAllInString)
        }
    }
