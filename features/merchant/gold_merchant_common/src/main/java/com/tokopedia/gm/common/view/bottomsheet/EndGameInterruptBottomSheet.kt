...
class EndGameInterruptBottomSheet : BaseBottomSheet() {
    ...
    private fun setTextView() {
        ...
        val descCardEndGameInterrupt =
            arguments?.getString(DESC_CARD_END_GAME_INTERRUPT_KEY).orEmpty()
        ...
        tvStatusEndGameInterrupt?.text =
            MethodChecker.fromHtml(getString(R.string.status_end_game_period_information_interrupt))
    }

    private fun btnGotoShopScore() {
    ...
    companion object {
        ...
        fun createInstance(
            titleEndGame: String,
            titleCardEndGame: String,
            descCardEndGame: String
        ): EndGameInterruptBottomSheet {
