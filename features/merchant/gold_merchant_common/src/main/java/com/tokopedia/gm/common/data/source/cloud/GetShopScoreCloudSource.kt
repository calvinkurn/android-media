...
class GetShopScoreCloudSource @Inject constructor(private val gmCommonApi: GMCommonApi) {

    fun getScoreInfo(shopId: String): Observable<ShopScoreResult> {
        return gmCommonApi.getShopScoreDetail(shopId).map {
            if (!it.isSuccessful) {
