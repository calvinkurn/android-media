...
object DynamicProductDetailMapper {
        ...
        data.forEachIndexed { index, component ->
            when (component.type) {
                ...
                ProductDetailConstant.PRODUCT_DETAIL -> {
                    listOfComponent.add(
                        ProductDetailInfoDataModel(
                            type = component.type,
                            name = component.componentName,
                            dataContent = mapToProductDetailInfoContent(component.componentData.firstOrNull())
                        )
                    )
                }
                ...
                ProductDetailConstant.INFO -> {
                    ...
                    val content =
                        if (contentData?.content?.isEmpty() == true) listOf(Content()) else contentData?.content
                    listOfComponent.add(
                        ProductGeneralInfoDataModel(
                            component.componentName, component.type, contentData?.applink
                                ?: "", contentData?.title ?: "",
    ...
    fun mapToDynamicProductDetailP1(data: PdpGetLayout): DynamicProductInfoP1 {
        ...
        val newDataWithMedia =
            contentData?.copy(media = mediaData.media, youtubeVideos = mediaData.youtubeVideos)
                ?: ComponentData()
    ...
    fun generateImageReviewUiData(data: ImageReviewGqlResponse.ProductReviewImageListQuery): ImageReview {
        ...
        data.detail?.images?.forEach {
            ...
            result.add(
                ImageReviewItem(
                    it.reviewID.toString(),
                    review.timeFormat?.dateTimeFmt1,
                    review.reviewer?.fullName,
                    it.uriThumbnail,
                    it.uriLarge,
                    review.rating,
                    data.isHasNext,
                    data.detail?.imageCountFmt,
                    data.detail?.imageCount
                )
            )
        }
    ...
    fun generateProductInfoParcel(
        ...
        return ProductInfoParcelData(
            ...
            data?.getProductImageUrl()
                ?: "",
            variantGuideLine,
            productInfoP1?.basic?.stats?.countTalk.toIntOrZero(),
            data?.youtubeVideos
                ?: listOf(),
            productInfoContent,
            forceRefresh,
            productInfoP1?.basic?.isTokoNow == true
        )
    }
    ...
    fun getAffiliateUIID(affiliateUniqueString: String, uuid: String): AffiliateUIIDRequest? {
        return if (affiliateUniqueString.isNotBlank()) AffiliateUIIDRequest(
            trackerID = uuid,
            uuid = affiliateUniqueString,
        ...
        return when {
            isParent -> {
                ...
                if (selectedChild == null) {
                    ...
                    AtcVariantMapper.mapVariantIdentifierWithDefaultSelectedToHashMap(
                        variantData,
