...
class PartialLatestTalkView  private constructor(private val view: View) {
    ...
    fun renderData(talk: Talk, totalTalk: Int, productShopId: Int, onDiscussionClicked: ((String)-> Unit)? = null){
        with(view){
            if (talk.id.isNotBlank()){
                ...
                txt_see_all_talk.setOnClickListener { onDiscussionClicked?.invoke(talk.id) }
                visible()
            } else {
                gone()
            }
