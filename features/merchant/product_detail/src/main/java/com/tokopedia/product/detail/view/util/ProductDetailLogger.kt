...
object ProductDetailLogger {
    ...
    fun logThrowable(throwable: Throwable, errorType: String, productId: String, userId: String, deviceId: String, extras: String = "") {
        if(throwable is MessageErrorException || (throwable is RuntimeException && isServerError(throwable))) {
            log(getMap(errorType, productId, userId, deviceId, throwable.localizedMessage ?: "", getTrimmedStackTrace(throwable), extras))
        }
    }

    fun logMessage(errorMessage: String, errorType: String, productId: String, userId: String, deviceId: String, extras: String = "") {
        GENERIC_ERROR_MESSAGES.forEach {
            if(errorMessage.toLowerCase().contains(it)) return
        }
        log(getMap(errorType, productId, userId, deviceId, errorMessage, "", extras))
    }

    private fun getMap(errorType: String, productId: String, userId: String, deviceId: String, errorMessage: String, stackTrace: String, extras: String = ""): Map<String, String> {
