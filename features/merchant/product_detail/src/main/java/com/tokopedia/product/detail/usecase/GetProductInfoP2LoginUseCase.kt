...
class GetProductInfoP2LoginUseCase @Inject constructor(
    ...
    private fun createWishListedRequest(): GraphqlRequest {
        ...
        return GraphqlRequest(
            ...
            mapOf(
                ProductDetailCommonConstant.PARAM_PRODUCT_ID to productId.orEmpty()
            )
        )
    }

    private fun createTopAdsManageRequest(): GraphqlRequest {
        val productId = requestParams.getString(ProductDetailCommonConstant.PARAM_PRODUCT_ID, "")
    ...
    private fun createTopAdsShop(): GraphqlRequest {
        val shopId = requestParams.getString(ProductDetailCommonConstant.PARAM_SHOP_IDS, "")
        val param =
            mapOf(ProductDetailCommonConstant.PARAM_APPLINK_SHOP_ID to shopId.toLongOrZero())
    ...
    private fun createShopFollow(): GraphqlRequest {
        val shopId = requestParams.getString(ProductDetailCommonConstant.PARAM_SHOP_IDS, "")
        val params = mapOf(
            ProductDetailCommonConstant.PARAM_SHOP_IDS to listOf(shopId.toLongOrZero()),
    ...
    override suspend fun executeOnBackground(): ProductInfoP2Login {
        ...
        val isWishlistedRequest = createWishListedRequest()
        val requests = mutableListOf(isWishlistedRequest)

        if (isShopOwner) {
            val topAdsManageRequest = createTopAdsManageRequest()
            val topAdsShopRequest = createTopAdsShop()
            requests.addAll(listOf(topAdsShopRequest, topAdsManageRequest))
        } else {
            val shopFollowRequest = createShopFollow()
            requests.addAll(listOf(shopFollowRequest))
        }

        try {
            val gqlResponse =
                graphqlRepository.response(requests, getCacheStrategy(fromCache = fromCache))

            if (gqlResponse.getError(WishlistStatus::class.java)?.isNotEmpty() != true) {
                p2Login.isWishlisted =
                    gqlResponse.getData<WishlistStatus>(WishlistStatus::class.java)
                        .isWishlisted == true
            } else {
            ...
            if (gqlResponse.getError(TopAdsGetProductManageResponse::class.java)
                    ?.isNotEmpty() != true
            ) {
                gqlResponse.doActionIfNotNull<TopAdsGetProductManageResponse> {
                    p2Login.topAdsGetProductManage =
                        gqlResponse.getData<TopAdsGetProductManageResponse>(
                            TopAdsGetProductManageResponse::class.java
            ...
            if (gqlResponse.getError(TopAdsGetShopInfo.Response::class.java)
                    ?.isNotEmpty() != true
            ) {
                gqlResponse.doActionIfNotNull<TopAdsGetShopInfo.Response> {
                    p2Login.topAdsGetShopInfo =
                        gqlResponse.getData<TopAdsGetShopInfo.Response>(TopAdsGetShopInfo.Response::class.java).topAdsGetShopInfo.topAdsShopData
                }
            }
            ...
            if (gqlResponse.getError(ProductShopFollowResponse::class.java)?.isNotEmpty() != true) {
                gqlResponse.doActionIfNotNull<ProductShopFollowResponse> {
                    p2Login.isFollow =
                        gqlResponse.getData<ProductShopFollowResponse>(ProductShopFollowResponse::class.java).shopInfoByID.result.firstOrNull()?.favoriteData?.alreadyFavorited
                            ?: 0
                }
            } else {
