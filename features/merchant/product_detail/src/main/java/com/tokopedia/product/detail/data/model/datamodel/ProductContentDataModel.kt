...
data class ProductContentDataModel(
    ...
    fun showTradeIn(): Boolean {
        return shouldShowTradein && data?.data?.campaign?.shouldShowRibbonCampaign == false && data?.isUpcomingNplType() == false
    }

    fun showCod(): Boolean {
        return shouldShowCod && !shouldShowTradein && data?.data?.campaign?.shouldShowRibbonCampaign == false && data?.isUpcomingNplType() == false
    }
