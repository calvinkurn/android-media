...
open class DynamicProductDetailFragment :
        ...
        if (productId != null || (productKey != null && shopDomain != null)) {
            context?.let {
                ...
                viewModel.getProductP1(
                    ...
                    affiliateUniqueString = affiliateUniqueId,
                    uuid = uuid,
                    urlQuery = urlQuery,
                    extParam = extParam
                )
    ...
    private fun onTradeinClickedAfter(componentTrackDataModel: ComponentTrackDataModel? = null) {
        ...
        DynamicProductDetailTracking.Click.trackTradein(
            viewModel.tradeInParams.usedPrice.toDouble(),
            viewModel.getDynamicProductInfoP1,
            componentTrackDataModel
        )
    ...
    override fun onImageReviewClick(
        ...
        context?.let {
            ...
            ImageReviewGalleryActivity.moveTo(
                context,
                ArrayList(listOfImageReview),
                position,
    ...
    override fun onImageClicked(position: Int) {
        ...
        activity?.let {
            ...
            val intent = ImagePreviewPdpActivity.createIntent(
                it,
    ...
    override fun onFabWishlistClicked(
        ...
        if (viewModel.isUserSessionActive) {
            if (isActive) {
                ...
                productInfo?.basic?.productID?.let {
                    toasterWishlistText =
                        if (isProductOos()) getString(R.string.toaster_success_add_wishlist_from_fab) else getString(
                            R.string.toaster_success_add_wishlist
                        )
                    addWishList()
                    productInfo.let {
    ...
    private fun observeImageVariantPartialyChanged() {
        viewLifecycleOwner.observe(viewModel.updatedImageVariant) {
            ...
            if (it.second.isNotEmpty()) {
                ...
                viewModel.updateDynamicProductInfoData(
                    VariantMapper.updateMediaToCurrentP1Data(
                        viewModel.getDynamicProductInfoP1,
                        mediaList
                    )
                )
    ...
    private fun observeP2Data() {
        viewLifecycleOwner.observe(viewModel.p2Data) {
            ...
            trackProductView(
                viewModel.tradeInParams.isEligible == ProductDetailConstant.ELIGIBLE_TRADE_IN,
                boeData.boType
            )
    ...
    private fun handleWishlistAction(productCardOptionsModel: ProductCardOptionsModel?) {
        ...
        if (wishlistResult.isUserLoggedIn) {
            if (wishlistResult.isSuccess) {
                ...
                view?.showToasterError(
                    if (wishlistResult.isAddWishlist) getString(com.tokopedia.topads.sdk.R.string.msg_error_add_wishlist) else getString(
                        com.tokopedia.topads.sdk.R.string.msg_error_remove_wishlist
                    )
                )
            }
        } else {
    ...
    private fun doAtc(buttonAction: Int) {
        ...
        context?.let {
            ...
            if (buttonActionType == ProductDetailCommonConstant.REMIND_ME_BUTTON) {
                toasterWishlistText =
                    getString(com.tokopedia.product.detail.common.R.string.toaster_success_add_wishlist_from_button)
                addWishList()
                return@let
            }
        ...
        if (viewModel.getShopInfo().isShopInfoNotEmpty()) {
            ...
            doActionOrLogin({
                ...
                activity?.let {
                    ...
                    VariantMapper.putChatProductInfoTo(
                        intent,
