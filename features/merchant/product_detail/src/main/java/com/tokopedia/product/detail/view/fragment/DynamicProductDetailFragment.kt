...
class DynamicProductDetailFragment : BaseListFragment<DynamicPdpDataModel, DynamicProductDetailAdapterFactoryImpl>(), DynamicProductDetailListener, ProductVariantListener, ProductAccessRequestDialogFragment.Listener {
    ...
    override fun onDiscussionSendQuestionClicked(componentTrackDataModel: ComponentTrackDataModel) {
        doActionOrLogin({
            viewModel.getDynamicProductInfoP1?.let {
                DynamicProductDetailTracking.Click.eventEmptyDiscussionSendQuestion(it, componentTrackDataModel, viewModel.userId, pdpUiUpdater?.productNewVariantDataModel?.isPartialySelected()?.not() ?: false)
            }
            goToWriteActivity()
        })
    ...
    private fun gotoEditProduct() {
        var id = viewModel.getDynamicProductInfoP1?.data?.variant?.parentID
        if (id.isNullOrBlank().or(id.toIntOrZero() <= 0)) {
            id = viewModel.getDynamicProductInfoP1?.parentProductId ?: return
        }
