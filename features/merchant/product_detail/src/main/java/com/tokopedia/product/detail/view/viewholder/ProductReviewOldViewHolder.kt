...
import kotlinx.android.synthetic.main.item_dynamic_review_old.view.container_image_review

class ProductReviewOldViewHolder(val view: View, val listener: DynamicProductDetailListener) :
        AbstractViewHolder<ProductMostHelpfulReviewDataModel>(view) {

    companion object {
        val LAYOUT = R.layout.item_dynamic_review_old
    ...
    private fun renderImageReview(imageReviews: List<ImageReviewItem>, totalRating: Int, ratingScore: Float, componentTrackDataModel: ComponentTrackDataModel) {
        ...
        view.image_review_list.adapter = ImageReviewAdapter(imageReviews.take(4).toMutableList(), showSeeAll, listener::onImageReviewClick, listener::onSeeAllLastItemImageReview,
