...
class LineGraphViewHolder(
    ...
    private fun getCustomYAxisLabel(element: LineGraphWidgetUiModel): List<AxisLabel> {
        return element.data?.yLabels?.map { AxisLabel(it.yVal.toFloat(), it.yLabel) }.orEmpty()
    }
