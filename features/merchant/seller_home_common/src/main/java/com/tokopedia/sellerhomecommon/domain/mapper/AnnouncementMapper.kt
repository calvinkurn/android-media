...
class AnnouncementMapper @Inject constructor() :
    BaseResponseMapper<GetAnnouncementDataResponse, List<AnnouncementDataUiModel>> {

    override fun mapRemoteDataToUiData(
        response: GetAnnouncementDataResponse,
        isFromCache: Boolean
    ): List<AnnouncementDataUiModel> {
        return response.fetchAnnouncementWidgetData?.data.orEmpty().map {
            AnnouncementDataUiModel(
                ...
                widgetDataSign = it.widgetDataSign
            )
        }
    }
}