 ...
 */
public class ShopEtalaseAdapter extends BaseListAdapter<ShopEtalaseViewModel, ShopEtalaseFactory> {
    public ShopEtalaseAdapter(ShopEtalaseFactory baseListAdapterTypeFactory,
                              OnAdapterInteractionListener<ShopEtalaseViewModel> onAdapterInteractionListener) {
        super(baseListAdapterTypeFactory, onAdapterInteractionListener);
    }

    public void deleteEtalase(String etalaseId) {
        List<ShopEtalaseViewModel> data = getData();

        for (int i = 0, sizei = data.size(); i < sizei; i++) {
            ShopEtalaseViewModel model = data.get(i);
