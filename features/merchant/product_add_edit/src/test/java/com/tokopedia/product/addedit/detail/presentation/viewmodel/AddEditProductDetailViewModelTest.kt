...
class AddEditProductDetailViewModelTest {
    ...
    fun `getAnnotationCategory should return unfilled data when productId is not provided`() = coroutineTestRule.runBlockingTest {
        ...
        if (result is Success) {
            ...
            val specificationText = viewModel.specificationText.value.orEmpty()
    ...
    @Test
    fun `updateSpecificationByAnnotationCategory should return empty when annotation category is not selected`() = coroutineTestRule.runBlockingTest {
        val annotationCategoryData = listOf(
                AnnotationCategoryData(
                        variant = "Merek",
                        data = listOf(
