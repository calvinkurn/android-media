...
class VideoLinkTypeFactory: BaseAdapterTypeFactory(){
    ...
    class VideoLinkViewHolder(val view: View?, private val listener: VideoLinkListener?)
        ...
        override fun bind(element: VideoLinkModel) {
            ...
            loadLayout(element.inputUrl, element.inputImage, element.inputTitle, element.inputDescription)
        ...
        private fun loadLayout(url: String,
                               imageUrl: String,
                               inputTitle: String,
                               inputDescription: String) {
            itemView.apply {
                cardThumbnail.visibility = if (url.isEmpty()) View.GONE else View.VISIBLE
