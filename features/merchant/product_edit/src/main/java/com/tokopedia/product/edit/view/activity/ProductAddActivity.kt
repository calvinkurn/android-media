...
open class ProductAddActivity : BaseProductAddEditActivity(), ProductAddImageView {
    ...
    fun handleImageUrlFromExternal() {
        showProgressDialog()
        val oriImageUrls = intent.getStringArrayListExtra(EXTRA_IMAGES)
        ...
        if (checkExplicitImageUrls()) {
            ...
            if (validateHasLoginAndShop()) {
                ...
                if (intent != null && intent.action != null) {
                    when (intent.action) {
                        Intent.ACTION_SEND -> ProductAddActivityPermissionsDispatcher.handleImageUrlImplicitSingleWithCheck(this)
                        Intent.ACTION_SEND_MULTIPLE -> ProductAddActivityPermissionsDispatcher.handleImageUrlImplicitMultipleWithCheck(this)
                    }
                }
            }
    ...
    private fun checkExplicitImageUrls(): Boolean {
        val intent = intent
        val imageUrlsTemp = intent.getStringArrayListExtra(EXTRA_IMAGES)
    ...
    private fun showProductAddFragment() {
        if (!this@ProductAddActivity.isFinishing) {
            dismissDialog()
            createProductAddFragment()
        }
    }
    ...
    object DeeplinkIntent{
        ...
        fun getCallingApplinkAddProductMainAppIntent(context: Context, extras: Bundle): Intent {
            ...
            if (SessionHandler.isUserHasShop(context)) {
                intent = Intent(context, ProductAddActivity::class.java)
            } else {
