...
class GetShopPenaltySummaryTypesUseCase @Inject constructor(
    ...
    companion object {
        ...
        @JvmStatic
        fun createParams(startDate: String, endDate: String): RequestParams =
    ...
    override suspend fun executeOnBackground(): ShopPenaltySummaryTypeWrapper {
        ...
        val shopScorePenaltyRequest = GraphqlRequest(
            SHOP_SCORE_PENALTY_TYPES_QUERY,
            ShopScorePenaltyTypesResponse::class.java,
            shopScorePenaltyParam
        )

        val shopPenaltySummaryParam = mapOf(
            INPUT_PARAM to ShopScorePenaltySummaryParam(
                startDate = startDate,
                endDate = endDate
            )
        )
        val shopPenaltyRequest = GraphqlRequest(
            SHOP_SCORE_PENALTY_SUMMARY_QUERY,
            ShopScorePenaltySummaryResponse::class.java,
            shopPenaltySummaryParam
        )
        ...
        try {
            ...
            if (gqlResponse.getError(ShopScorePenaltyTypesResponse::class.java).isNullOrEmpty()) {
                val penaltyTypeData =
                    gqlResponse.getData<ShopScorePenaltyTypesResponse>(ShopScorePenaltyTypesResponse::class.java).shopScorePenaltyTypes
                shopPenaltySummaryTypeWrapper.shopScorePenaltyTypesResponse = penaltyTypeData
            } else {
                val penaltyTypeError =
                    gqlResponse.getError(ShopScorePenaltyTypesResponse::class.java)
