...
class GetBroadcasterShopConfigUseCase(private val gqlUseCase: MultiRequestGraphqlUseCase) : UseCase<BroadcasterConfig.Response>() {
    ...
    """
    val request by lazy{
        GraphqlRequest(query, BroadcasterConfig.Response::class.java, params.parameters)
    }

    override suspend fun executeOnBackground(): BroadcasterConfig.Response {
        ...
        if (error == null || error.isEmpty()) {
            return (gqlResponse.getData(BroadcasterConfig.Response::class.java) as BroadcasterConfig.Response)
        } else {
