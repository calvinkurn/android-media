...
class ManageProductMultiLocationVariantActivity : BaseSimpleActivity() {
    ...
    companion object {
        @JvmStatic
        fun start(context: Context, product: ReservedProduct.Product) : Intent{
    ...
    private val variantPositionOnProduct by lazy {
        intent?.extras?.getInt(BundleConstant.VARIANT_POSITION).toZeroIfNull()
    }
    override fun getLayoutRes() = R.layout.stfs_activity_manage_product_multi_location_variant
    override fun getNewFragment() = ManageProductMultiLocationVariantFragment.newInstance(product, variantPositionOnProduct)
