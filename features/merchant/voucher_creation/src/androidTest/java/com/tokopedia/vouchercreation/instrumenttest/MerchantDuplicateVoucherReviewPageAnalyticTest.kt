...
class MerchantDuplicateVoucherReviewPageAnalyticTest {
    ...
    companion object {
        private const val DUPLICATE_VOUCHER_REVIEW_PAGE_OPEN =
            "tracker/merchant/voucher_creation/mvc_duplicate_voucher_page_open.json"
        private const val IS_DUPLICATE = "is_duplicate"
        private const val DUPLICATE_VOUCHER = "duplicate_voucher"
    }

    @get:Rule
    var activityRule: IntentsTestRule<CreateMerchantVoucherStepsActivity> =
        IntentsTestRule(CreateMerchantVoucherStepsActivity::class.java, false, false)
    ...
    fun beforeTest() {
        ...
        activityRule.launchActivity(
            Intent().putExtra(IS_DUPLICATE, true).putExtra(DUPLICATE_VOUCHER, voucherUiModel)
        )
    }
