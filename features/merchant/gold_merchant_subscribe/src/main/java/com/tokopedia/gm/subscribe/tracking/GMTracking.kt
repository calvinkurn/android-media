...
class GMTracking(){
    ...
    fun sendClickHamburgerMenuEvent(menuName: String){
        TrackApp.getInstance()?.getGTM()?.sendGeneralEvent(TrackAppUtils.gtmData(CLICK_HAMBURGER_MENU_EVENT,
    ...
    fun sendClickManageProductDialogEvent(isSubscribing: Boolean){
        TrackApp.getInstance()?.getGTM()?.sendGeneralEvent(TrackAppUtils.gtmData(CLICK_GM_EVENT, CLICK_GM_CATEGORY,
