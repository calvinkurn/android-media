...
class SnapPagerScrollListener(private val snapHelper: PagerSnapHelper, private val listenerPreview: ReviewImagePreviewSwipeListener) : RecyclerView.OnScrollListener() {
    ...
    private fun notifyListenerIfNeeded(newSnapPosition: Int) {
        if (snapPosition != newSnapPosition) {
            if (NOTIFY_ON_INIT && !hasItemPosition()) {
                listenerPreview.onImageSwiped(snapPosition, newSnapPosition)
