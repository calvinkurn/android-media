...
import com.tokopedia.centralizedpromoold.view.adapter.CentralizedPromoAdapterOld
...
abstract class BasePartialListViewOld<T : BaseUiListModel<V>, F : BaseAdapterTypeFactory, V : Visitable<F>>(
    binding: FragmentCentralizedPromoOldBinding,
    typeFactory: F,
    coachMarkListener: CoachMarkListener,
    showCoachMark: Boolean
) : BasePartialViewOld<T>(coachMarkListener, binding, showCoachMark) {

    protected val adapter by lazy {
        CentralizedPromoAdapterOld<V, F>(typeFactory, ::onResultDispatched)
    }
