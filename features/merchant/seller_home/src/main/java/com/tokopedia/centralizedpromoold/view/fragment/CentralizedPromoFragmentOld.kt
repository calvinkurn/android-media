package com.tokopedia.centralizedpromoold.view.fragment
...
import com.tokopedia.centralizedpromoold.view.fragment.partialview.BasePartialViewOld
import com.tokopedia.centralizedpromoold.view.fragment.partialview.PartialCentralizedPromoCreationViewOld
import com.tokopedia.centralizedpromoold.view.fragment.partialview.PartialCentralizedPromoOnGoingPromoViewOld
...
class CentralizedPromoFragmentOld : BaseDaggerFragment(),
    PartialCentralizedPromoOnGoingPromoViewOld.RefreshButtonClickListener, CoachMarkListener {
    ...
    companion object {
        ...
        @JvmStatic
        fun createInstance(): CentralizedPromoFragmentOld = CentralizedPromoFragmentOld()
    }
    ...
    private val adapterTypeFactory by lazy {
        CentralizedPromoAdapterTypeFactoryOld(
    ...
    private val centralizedPromoViewModelOld by lazy {
        ViewModelProvider(this, viewModelFactory).get(CentralizedPromoViewModelOld::class.java)
    }
    ...
    private var binding by autoClearedNullable<FragmentCentralizedPromoOldBinding>()
    ...
    override fun onCreateView(
        ...
        binding = FragmentCentralizedPromoOldBinding.inflate(inflater, container, false)
        return binding?.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        refreshLayout()
        CentralizedPromoTrackingOld.sendOpenScreenEvent(userSession.isLoggedIn, userSession.userId)
    }
    ...
    private fun createOnGoingPromoView(): PartialCentralizedPromoOnGoingPromoViewOld? {
        return binding?.let {
            PartialCentralizedPromoOnGoingPromoViewOld(
    ...
    private fun createPromoRecommendationView(): PartialCentralizedPromoCreationViewOld? {
        return binding?.let {
            PartialCentralizedPromoCreationViewOld(
                binding = it,
                adapterTypeFactory = adapterTypeFactory,
                coachMarkListener = this,
                showCoachMark = sharedPref.getBoolean(
    ...
    private fun getLayoutData(vararg layoutTypes: LayoutType) {
        centralizedPromoViewModelOld.getLayoutData(*layoutTypes)
    }

    private fun observeGetLayoutDataResult() {
        centralizedPromoViewModelOld.getLayoutResultLiveData.observe(viewLifecycleOwner, Observer {
            it.forEach { entry ->
                when (val value = entry.value) {
                    is Success -> value.onSuccessGetLayoutData<BaseUiModel, BasePartialViewOld<BaseUiModel>>(
                        entry.key
                    )
    ...
    private inline fun <reified D : BaseUiModel, reified V : BasePartialViewOld<D>> Success<BaseUiModel>.onSuccessGetLayoutData(
    ...
    private fun trackFlashSaleTokoClick(cardTitle: String) {
        CentralizedPromoTrackingOld.sendClickFlashSaleToko(cardTitle, userSession.shopId)
    }

    private fun trackFreeShippingImpression() {
        centralizedPromoViewModelOld.trackFreeShippingImpression()
    }

    private fun trackFreeShippingClick() {
        centralizedPromoViewModelOld.trackFreeShippingClick()
    }

    private fun trackProductCouponImpression() {
        CentralizedPromoTrackingOld.sendImpressionProductCouponPromoCreation(userSession.shopId)
    }

    private fun trackProductCouponClick() {
        CentralizedPromoTrackingOld.sendClickProductCouponPromoCreation(userSession.shopId)
    }

    private fun trackProductCouponOngoingPromoClick(campaignName: String) {
        CentralizedPromoTrackingOld.sendClickProductCouponOngoingPromo(campaignName, userSession.shopId)
    }

    private fun trackTokoMemberImpression() {
        CentralizedPromoTrackingOld.sendViewTokoMember(userSession.shopId)
    }

    private fun trackTokoMemberClicked() {
        CentralizedPromoTrackingOld.sendClickTokoMember(userSession.shopId)
    }
}
