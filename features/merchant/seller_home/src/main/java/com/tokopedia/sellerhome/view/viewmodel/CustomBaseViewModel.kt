...
abstract class CustomBaseViewModel(private val dispatcher: SellerHomeCoroutineDispatcher) : BaseViewModel(dispatcher.main()) {
    ...
    protected fun <T : Any> CoroutineScope.executeCall(
        ...
        launchCatchError(
                context = context,
                block = {
                    liveData.value = Success(withContext(dispatcher.io()) {
                        block()
                    })
                },
                onError = {
