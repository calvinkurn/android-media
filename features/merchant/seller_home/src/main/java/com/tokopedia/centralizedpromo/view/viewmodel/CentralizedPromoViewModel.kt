...
class CentralizedPromoViewModel @Inject constructor(
    ...
    private suspend fun getPostList(): Result<BaseUiModel> {
        return try {
            getPostUseCase.params = GetPostUseCase.getRequestParams(if (shopId.isBlank()) 0 else shopId.toIntOrZero(), listOf(GetPostUseCase.DATA_KEY_SELLER_INFO_POST), startDate, endDate)
