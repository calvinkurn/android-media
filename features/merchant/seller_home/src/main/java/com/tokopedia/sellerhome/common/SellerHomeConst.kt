package com.tokopedia.sellerhome.common

import com.tokopedia.imageassets.ImageUrl

/**
 * Created By @ilhamsuaib on 08/09/21
 */

object SellerHomeConst {

    const val EMPTY_STRING = ""
    const val SHOP_SHARE_DEFAULT_CHANNEL = "default"
    const val SHOP_SHARE_OTHERS_CHANNEL = "others"
    const val SHOP_SHARE_GQL_TRACKER_ACTION = "ShopSharing"
    const val SHOP_SHARE_GQL_TRACKER_SOURCE = "seller-home"

    object Images {
        const val IMG_ERROR_500 = ImageUrl.GLOBAL_ERROR_500
        const val IMG_NEW_SELLER_LEFT = ImageUrl.IMG_NEW_SELLER_LEFT
        const val IMG_NEW_SELLER_RIGHT = ImageUrl.IMG_NEW_SELLER_RIGHT
    }
}