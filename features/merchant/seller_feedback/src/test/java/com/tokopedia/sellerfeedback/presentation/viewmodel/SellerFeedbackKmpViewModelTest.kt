...
import com.tokopedia.kmp.seller.feedback.data.param.FeedbackParam
...
class SellerFeedbackKmpViewModelTest : SellerFeedbackViewKmpModelTestFixture() {
    ...
    fun `when upload image and submit feedback success will return submit result success`() {
        ...
        val sellerFeedback = FeedbackParam(
            shopId = anyLong(),
            score = anyString(),
            type = anyString(),
            page = anyString(),
        ...
        coEvery { submitFeedbackUseCase.execute(sellerFeedback) } returns wrapper

        viewModel.setImages(images)
        viewModel.submitFeedbackKmp(sellerFeedback)
        viewModel.getSubmitResultKmp().verifyValueEquals(SubmitResult.Success)

        coVerify { uploaderUseCase(uploadParam1) }
        coVerify { uploaderUseCase(uploadParam2) }
        coVerify { uploaderUseCase(uploadParam3) }
        coVerify { submitGlobalFeedback.exe() }
    }
