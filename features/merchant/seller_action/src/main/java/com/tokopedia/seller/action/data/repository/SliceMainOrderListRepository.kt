...
class SliceMainOrderListRepository @Inject constructor(
    ...
    private val mOrderListLiveData = MutableLiveData<Result<Pair<Uri, List<SellerActionOrderList.Data.OrderList.Order>>>>()

    fun getOrderList(sliceUri: Uri): LiveData<Result<Pair<Uri, List<SellerActionOrderList.Data.OrderList.Order>>>> {
    ...
    private fun loadMainOrderList(sliceUri: Uri) {
        GlobalScope.launch(dispatcher.io()) {
            try {
                ...
                mOrderListLiveData.value = Fail(SellerActionException(sliceUri, ex.message.orEmpty()))
            }
        }
    }
    ...
    private suspend fun getSliceMainOrderList(sliceUri: Uri) {
        ...
        with(sliceMainOrderListUseCase) {
            ...
            mOrderListLiveData.value = Success(Pair(sliceUri, executeOnBackground()))
        }
    }
}