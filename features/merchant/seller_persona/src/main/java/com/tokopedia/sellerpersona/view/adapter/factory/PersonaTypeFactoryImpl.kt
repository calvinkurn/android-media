...
import com.tokopedia.sellerpersona.view.adapter.viewholder.PersonaTypeLoadingViewHolder
...
class PersonaTypeFactoryImpl(
    ...
    override fun type(model: PersonaTypeLoadingUiModel): Int {
        return PersonaTypeLoadingViewHolder.RES_LAYOUT
    }

    override fun createViewHolder(parent: View, type: Int): AbstractViewHolder<out Visitable<*>> {
        return when (type) {
            PersonaTypeViewHolder.RES_LAYOUT -> PersonaTypeViewHolder(listener, parent)
            PersonaTypeLoadingViewHolder.RES_LAYOUT -> PersonaTypeLoadingViewHolder(parent)
