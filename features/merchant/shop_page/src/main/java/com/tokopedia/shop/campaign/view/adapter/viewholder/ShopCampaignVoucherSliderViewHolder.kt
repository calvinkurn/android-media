...
class ShopCampaignVoucherSliderViewHolder(
    ...
    private var merchantVoucherReload: CardView? = binding?.voucherSliderReload?.merchantVoucherReload
    ...
    private fun getVoucherListModel(uiModel: ShopWidgetVoucherSliderUiModel): List<Any> {
        return mutableListOf<Any>().apply {
            val voucherList = uiModel.listVoucher.take(5)
            addAll(voucherList)
            if (uiModel.listVoucher.size > 5) {
                add(ExclusiveLaunchMoreVoucherUiModel(
                    totalRemainingVoucher = uiModel.listVoucher.size - 5,
