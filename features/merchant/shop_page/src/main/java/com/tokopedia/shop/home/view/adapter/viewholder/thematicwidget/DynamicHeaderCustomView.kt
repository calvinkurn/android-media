package com.tokopedia.shop.home.view.adapter.viewholder.thematicwidget
...
class DynamicHeaderCustomView : FrameLayout {
    ...
    fun setModel(model: ThematicWidgetUiModel, header: BaseShopHomeWidgetUiModel.Header, listener: HeaderCustomViewListener? = null) {
        this.listener = listener
        setHeaderComponent(model, header)
    }

    private fun setHeaderComponent(model: ThematicWidgetUiModel, header: BaseShopHomeWidgetUiModel.Header) {
        setupUi()
        setTitle(model.campaignName)
        setSubtitle(model.campaignSubName, model.statusCampaign, model.endDate)
        setSeeAllAppLink(header.ctaText, header.ctaLink)
    }
    ...
    fun setShopPageCta(model: BaseShopHomeWidgetUiModel.Header) {
        val ctaTextLink = model.ctaLink
        tpSeeAll?.hide()
        if (ctaTextLink.isNotBlank()) {
