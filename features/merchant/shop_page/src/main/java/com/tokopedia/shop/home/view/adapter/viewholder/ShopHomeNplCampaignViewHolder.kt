...
class ShopHomeNplCampaignViewHolder(
    ...
    private fun setBannerImage(model: ShopHomeNewProductLaunchCampaignUiModel) {
        ...
        itemView.banner_background?.apply {
            layoutParams.height = if (statusCampaign == StatusCampaign.FINISHED.statusCampaign) {
...
//        val currentTime = Date(System.currentTimeMillis())
    ...
    private fun setCampaignInterest(
        ...
        if (totalNotifyWording.isNotEmpty() && statusCampaign.toLowerCase() == StatusCampaign.UPCOMING.statusCampaign.toLowerCase()) {
            val totalCampaignInterestString = totalNotifyWording.replace("{{number_of_users}}", totalNotify.thousandFormatted())
