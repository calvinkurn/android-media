...
class ShopProductSearchPageAnalyticTest {
    ...
    companion object{
        private const val SHOP_SEARCH_PRODUCT_CLICK_DI_ETALASE_TOKO_INI_MATCHER_PATH = "tracker/shop/shop_page_click_search_button_tracker.json"
    ...
    fun beforeTest() {
        ...
        activityRule.launchActivity(Intent().apply {
            ...
            putExtra(KEY_SHOP_NAME, "Shop  name")
            putExtra(KEY_IS_OFFICIAL, true)
            putExtra(KEY_IS_GOLD_MERCHANT, true)
            putExtra(KEY_SHOP_ATTRIBUTION, "")
            putExtra(KEY_KEYWORD, "")
            putExtra(KEY_SHOP_REF, "")
        })
    }
    ...
    fun testClickOnDiEtalaseTokoIni() {
        waitForData()
        Espresso.onView(CommonMatcher.firstView(withId(R.id.editTextSearchProduct))).perform(ViewActions.typeText("Sabun"))
