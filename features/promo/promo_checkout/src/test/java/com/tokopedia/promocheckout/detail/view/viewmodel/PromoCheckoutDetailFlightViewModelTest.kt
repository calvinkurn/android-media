...
class PromoCheckoutDetailFlightViewModelTest{
    @get:Rule
    ...
    private lateinit var viewModel: PromoCheckoutDetailFlightViewModel

    @Before
    fun setup(){
        viewModel = PromoCheckoutDetailFlightViewModel(dispatcher, flightCheckVoucherUsecase, getDetailPromoCheckoutUseCase, travelCancelVoucherUseCase)
    }
    ...
    fun checkPromoCode_isSuccess(){
        ...
        assert(viewModel.showProgressLoadingPromoFlight.value is Boolean)
        assertEquals(Success(DUMMY_DATA_UI_MODEL), (viewModel.flightCheckVoucherResult.value) as Success)
        assertEquals(false, viewModel.showProgressLoadingPromoFlight.value)
    }
