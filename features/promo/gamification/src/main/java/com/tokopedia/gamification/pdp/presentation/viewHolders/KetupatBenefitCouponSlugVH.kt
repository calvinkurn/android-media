...
class KetupatBenefitCouponSlugVH(itemView: View) : AbstractViewHolder<KetupatBenefitCouponSlugVHModel>(itemView) {
    ...
    override fun bind(element: KetupatBenefitCouponSlugVHModel?) {
        ...
        element?.value.let {
            ...
            it?.tokopointsCouponListStack?.tokopointsCouponDataStack?.forEach{
                it?.let { it1 -> KetupatBenefitCouponSlugItemVHModel(it1) }
            ...
            itemView.findViewById<RecyclerView>(gamificationR.id.rv_benefit_coupon_slug).adapter = adapter
        }
    }
}
