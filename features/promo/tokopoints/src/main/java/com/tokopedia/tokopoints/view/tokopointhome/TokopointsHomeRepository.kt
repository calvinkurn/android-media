...
class TokopointsHomeRepository @Inject constructor(@Named(TP_GQL_TOKOPOINT_DETAIL_NEW) private val tp_gql_tokopoint_detail_new: String,
    ...
    @Inject
    lateinit var mGetTokenDetailUsecase: MultiRequestGraphqlUseCase
    ...
    suspend fun getTokenDetail() = withContext(Dispatchers.IO) {
        mGetTokenDetailUsecase.clearRequest()
        //Lucky egg
        val request2 = GraphqlRequest(tp_gql_lucky_egg_details,
                TokenDetailOuter::class.java, false)
        mGetTokenDetailUsecase.addRequest(request2)
        mGetTokenDetailUsecase.executeOnBackground()
    }
