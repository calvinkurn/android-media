...
class SectionHorizontalCarousel11ViewBinder(val block: SectionContent)
...
class SectionHorizontalCarousel11VH(val view: View) : RecyclerView.ViewHolder(view) {
    fun bind(content: SectionContent) {
        ...
        if (!TextUtils.isEmpty(content.sectionTitle)) {
            view.findViewById<View>(R.id.text_title_11).visibility = View.VISIBLE
        ...
        if (!TextUtils.isEmpty(content.sectionSubTitle)) {
            view.findViewById<View>(R.id.text_sub_title_11).visibility = View.VISIBLE
