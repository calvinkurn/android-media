...
class StaticMenuGenerator @Inject constructor(val context: Context) {
    ...
    fun generateUserSettingMenu(): SettingDataView {
        return SettingDataView(context?.getString(R.string.menu_account_section_title_account_setting), mutableListOf(
                ...
                CommonDataView(applink = ApplinkConstInternalGlobal.PRIVACY_ACCOUNT, title = context?.getString(R.string.menu_account_title_privacy_account), body = context?.getString(R.string.menu_account_desc_privacy_account), type = CommonViewHolder.TYPE_DEFAULT, icon = IconUnify.GLOBE, id = AccountConstants.SettingCode.SETTING_PRIVACY_ACCOUNT),
