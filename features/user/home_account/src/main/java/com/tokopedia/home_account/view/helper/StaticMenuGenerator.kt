...
class StaticMenuGenerator @Inject constructor(val context: Context) {
    ...
    fun generateApplicationSettingMenu(
        ...
        listSetting.addAll(mutableListOf(
                ...
                CommonDataView(id = AccountConstants.SettingCode.SETTING_APP_ADVANCED_SCREEN_RECORD, title = context?.getString(R.string.menu_account_title_screen_recorder), body = context?.getString(R.string.menu_account_desc_screen_recorder), type = CommonViewHolder.TYPE_DEFAULT, icon = IconUnify.CAMERA)
        ))
