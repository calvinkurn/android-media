...
import com.tokopedia.profilecompletion.profilecompletion.view.customview.ProfileCompletionTextDrawable
import com.tokopedia.profilecompletion.profilecompletion.view.listener.ProfileCompletionContract
import com.tokopedia.profilecompletion.profilecompletion.view.util.ProfileCompletionProgressBarAnimation
...
class ProfileCompletionFragment : BaseDaggerFragment(), ProfileCompletionContract.View {
    ...
    private fun initObserver() {
        pinViewModel?.getStatusPinResponse?.observe(
            viewLifecycleOwner,
            Observer { statusPinDataResult: Result<StatusPinData>? ->
        ...
        userInfoViewModel.userProfileInfo.observe(viewLifecycleOwner, Observer {
            when (it) {
                ...
                is Fail -> {
                    onErrorGetUserInfo(ErrorHandler.getErrorMessage(context, it.throwable))
                }
            }
        })
    }
    ...
    private fun onSuccessGetStatusPin(statusPinData: StatusPinData) {
        ...
        if (!statusPinData.isRegistered &&
            ...
            val intent =
                RouteManager.getIntent(context, ApplinkConstInternalUserPlatform.ADD_PIN_ONBOARDING)
    ...
    private fun initialVar() {
        animationProfileCompletion =
            progressBarTop?.let { ProfileCompletionProgressBarAnimation(it) }
        filled = "filled"
        pair =
    ...
    private fun loadFragment(
        profileCompletionDataView: ProfileCompletionDataView,
        pair: Pair<Int, Int>?
    ) {
    ...
    private fun chooseFragment(profileCompletionDataView: ProfileCompletionDataView) {
        if (checkingIsEmpty(profileCompletionDataView.gender.toString())) {
            ...
            val verifCompletionFragment =
                ProfileCompletionPhoneVerificationFragment.createInstance(this)
            fragmentTransaction?.replace(
                R.id.fragment_container,
                verifCompletionFragment,
