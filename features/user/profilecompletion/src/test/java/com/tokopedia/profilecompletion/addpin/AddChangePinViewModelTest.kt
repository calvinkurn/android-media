...
//    @Test
//    fun `on Error Create Pin V2 - throw exception`() {
//	/* When */
//	coEvery { createPinV2UseCase(any()) } throws mockThrowable
//
//	viewModel.addPinV2("", "", "")
//
//	/* Then */
//	assert(viewModel.mutatePin.value is Fail)
//    }
    ...
    fun `check pin v2 - success`() {
        ...
        /* Then */
        assertTrue((viewModel.checkPinResponse.value as Success).data.valid)
        verify { checkPinObserver.onChanged(any<Success<CheckPinData>>()) }
    }
	...
	val errorMsg = "error"
	val checkPinData = CheckPinData(valid = false, errorMessage = errorMsg)
	val checkPinV2Response = CheckPinV2Response(checkPinData)
	...
	/* Then */
	assert((viewModel.checkPinResponse.value as Success).data.errorMessage == errorMsg)
	verify { checkPinObserver.onChanged(any<Success<CheckPinData>>()) }
    }
	...
	val checkPinData = CheckPinData(valid = false, errorMessage = "")
	val checkPinV2Response = CheckPinV2Response(checkPinData)
