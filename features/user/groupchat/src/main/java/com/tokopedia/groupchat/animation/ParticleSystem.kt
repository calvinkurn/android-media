...
class ParticleSystem private constructor(parentView: ViewGroup, private val mMaxParticles: Int, private val mTimeToLive: Long) {
    ...
    private var mParticlesPerMillisecond: Float = 0.toFloat()
    private var mActivatedParticles: Int = 0
    ...
    private var mEmitterXMin: Int = 0
    private var mEmitterXMax: Int = 0
    private var mEmitterYMin: Int = 0
    private var mEmitterYMax: Int = 0
    ...
    private class ParticleTimerTask(ps: ParticleSystem) : TimerTask() {
        ...
        override fun run() {
            if (mPs.get() != null) {
                val ps = mPs.get()
                ps?.let {
                    it.onUpdate(ps.mCurrentTime)
                    it.mCurrentTime += TIMER_TASK_INTERVAL
                }
            }
        }
    }
     ...
     */
    constructor(parentView: ViewGroup, maxParticles: Int, drawable: Drawable, timeToLive: Long) : this(parentView, maxParticles, timeToLive) {
    ...
    constructor(parentView: ViewGroup, drawables: ArrayList<Drawable>, timeToLive: Long) : this(parentView, 2 * drawables.size, timeToLive) {
     ...
     * @param timeToLive    The time to live for the particles
     */
    constructor(a: Activity, maxParticles: Int, drawableRedId: Int, timeToLive: Long) : this(a, maxParticles, a.resources.getDrawable(drawableRedId), timeToLive, android.R.id.content) {}
     ...
     */
    constructor(a: Activity, maxParticles: Int, drawableRedId: Int, timeToLive: Long, parentViewId: Int) : this(a, maxParticles, a.resources.getDrawable(drawableRedId), timeToLive, parentViewId) {}
     ...
     * @param parentViewId The view Id for the parent of the particle system
     */
    @JvmOverloads constructor(a: Activity, maxParticles: Int, drawable: Drawable, timeToLive: Long, parentViewId: Int = android.R.id.content) : this(a.findViewById<ViewGroup>(parentViewId), maxParticles, drawable, timeToLive) {}
     ...
     * @param parentViewId The view Id for the parent of the particle system
     */
    @JvmOverloads constructor(a: Activity, maxParticles: Int, bitmap: Bitmap, timeToLive: Long, parentViewId: Int = android.R.id.content) : this(a.findViewById<View>(parentViewId) as ViewGroup, maxParticles, timeToLive) {
        for (i in 0 until mMaxParticles) {
            mParticles.add(Particle(bitmap))
        }
    }
     ...
     * @param parentViewId The view Id for the parent of the particle system
     */
    @JvmOverloads constructor(a: Activity, maxParticles: Int, animation: AnimationDrawable, timeToLive: Long, parentViewId: Int = android.R.id.content) : this(a.findViewById<ViewGroup>(parentViewId), maxParticles, timeToLive) {
        // Create the particles
        for (i in 0 until mMaxParticles) {
    ...
    fun setSpeedRange(speedMin: Float, speedMax: Float): ParticleSystem {
        mInitializers.add(SpeedModuleAndRangeInitializer(dpToPx(speedMin), dpToPx(speedMax), 0, 360))
        return this
    }
     ...
     * @return This.
     */
    fun setSpeedModuleAndAngleRange(speedMin: Float, speedMax: Float, minAngle: Int, maxAngle: Int): ParticleSystem {
        ...
        mInitializers.add(SpeedModuleAndRangeInitializer(dpToPx(speedMin), dpToPx(speedMax), minAngle, maxAngle))
        return this
    }
     ...
     * @return This.
     */
    fun setSpeedByComponentsRange(speedMinX: Float, speedMaxX: Float, speedMinY: Float, speedMaxY: Float): ParticleSystem {
     ...
     * @return
     */
    fun setAccelerationModuleAndAndAngleRange(minAcceleration: Float, maxAcceleration: Float, minAngle: Int, maxAngle: Int): ParticleSystem {
        mInitializers.add(AccelerationInitializer(dpToPx(minAcceleration), dpToPx(maxAcceleration),
                minAngle, maxAngle))
        return this
    }
    ...
    fun setFadeOut(milisecondsBeforeEnd: Long, interpolator: Interpolator): ParticleSystem {
        mModifiers.add(AlphaModifier(255, 0, mTimeToLive - milisecondsBeforeEnd, mTimeToLive, interpolator))
        return this
    }
     ...
     */
    @JvmOverloads
    fun oneShot(emitter: View, numParticles: Int, interpolator: Interpolator = LinearInterpolator()) {
    ...
    private fun startAnimator(interpolator: Interpolator, animnationTime: Long) {
        mAnimator = ValueAnimator.ofInt(0, animnationTime.toInt())
        mAnimator?.duration = animnationTime
    ...
    private fun changeDrawablesToParticles(drawableList: ArrayList<Drawable>) {
        val particleList = ArrayList<Particle>()
        for (i in drawableList.indices) {
            val drawable = drawableList[i]
            if (drawable is AnimationDrawable) {
                particleList.add(AnimatedParticle(drawable))
            } else {
                val bitmap: Bitmap
                if (drawable is BitmapDrawable) {
                    bitmap = drawable.bitmap
                ...
                particleList.add(Particle(bitmap))
            }
        }
    }
