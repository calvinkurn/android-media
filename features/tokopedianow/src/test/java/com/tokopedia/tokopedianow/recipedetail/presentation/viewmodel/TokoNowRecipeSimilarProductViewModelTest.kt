...
class TokoNowRecipeSimilarProductViewModelTest : TokoNowRecipeSimilarProductViewModelTestFixture() {
    ...
    @Test
    fun `given recipe list when onSuccessGetMiniCartData should map atc quantity to recipe list`() {
        val miniCartItems = mapOf(
            MiniCartItemKey("2148241524") to MiniCartItem.MiniCartItemProduct(
                productId = "2148241524",
                quantity = 100
    ...
    @Test
    fun `given layout list throws error when onSuccessGetMiniCartData data should do nothing`() {
    ...
    @Test
    fun `when get miniCartSource should return TokonowRecipe`() {
        assertEquals(MiniCartSource.TokonowRecipe, viewModel.miniCartSource)
    }
}
