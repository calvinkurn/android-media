...
class ContentDetailFragment :
    ...
    private fun onShareProduct(
        ...
        val desc = context?.getString(com.tokopedia.content.common.R.string.cdp_share_default_text)?.let {
            String.format(
                it,
                item.product.name,
                "%s",
    ...
    private fun showToastOnSuccessReminderSetForFSTorRS(card: FeedXCard) {
        when {
            card.campaign.reminder is FeedASGCUpcomingReminderStatus.On && card.campaign.isFlashSaleToko -> showToast(
                getString(com.tokopedia.content.common.R.string.feed_asgc_reminder_activate_fst_message),
                Toaster.TYPE_NORMAL
            )
            card.campaign.reminder is FeedASGCUpcomingReminderStatus.On && card.campaign.isRilisanSpl -> showToast(
                getString(com.tokopedia.content.common.R.string.feed_asgc_reminder_activate_rs_message),
                Toaster.TYPE_NORMAL
            )
            ...
            when (it) {
                is ContentDetailResult.Success -> {
                    ...
                    val toastMessage = if (it.data.isFollow) {
                        getString(com.tokopedia.content.common.R.string.feed_unfollow_ugc_success_toaster_text)
                    } else {
                        getString(com.tokopedia.content.common.R.string.feed_follow_ugc_success_toaster_text)
                    }
