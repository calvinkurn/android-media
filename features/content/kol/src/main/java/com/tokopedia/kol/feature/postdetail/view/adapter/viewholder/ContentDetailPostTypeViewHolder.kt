...
//                )
    ...
    private fun bindPublishedAt(publishedAt: String) {
        ...
        val spannableString = SpannableString(
            String.format(
                context.getString(commonR.string.feed_header_time_new),
                avatarDate
            )
        )
    ...
    fun bindHeader(
        ...
        val contentSubInfoValue = if (feedXCard.isTypeProductHighlight) {
            when (feedXCard.type) {
                ASGC_NEW_PRODUCTS -> context.getString(commonR.string.feeds_asgc_new_product_text)
                ASGC_RESTOCK_PRODUCTS -> context.getString(
                    commonR.string.feeds_asgc_restock_text
                )
                ASGC_DISCOUNT_TOKO -> context.getString(commonR.string.feed_asgc_diskon_toko)
                ASGC_FLASH_SALE_TOKO -> context.getString(commonR.string.feed_asgc_flash_sale_toko)
                ASGC_RILISAN_SPECIAL -> context.getString(commonR.string.feed_asgc_rilisan_special)
                else -> String.EMPTY
            }
        } else {
            if (count >= FOLLOW_COUNT_THRESHOLD) {
                String.format(
                    context.getString(commonR.string.feed_header_follow_count_text),
                    count.productThousandFormatted()
                )
            } else {
                context.getString(commonR.string.feed_header_follow_count_less_text)
            }
        }
        bindContentSubInfo(
            shouldShow = (
        ...
        if (!followers.transitionFollow && followers.isFollowed) {
            ...
            val textFollowAction = if (followers.isFollowed) {
                context.getString(commonR.string.kol_action_following_color)
            } else {
                context.getString(commonR.string.kol_action_follow_color)
            }

            this.authorFollowAction.text = MethodChecker.fromHtml(
                "${context.getString(commonR.string.feed_header_separator)}$textFollowAction"
        ...
        if (view.count != 0) {
            ...
            likedText.text =
                MethodChecker.fromHtml(
                    context.getString(
                        commonR.string.feed_component_viewed_count_text,
        ...
        if (like.likedBy.isNotEmpty() || like.count != 0) {
            ...
            if (like.likedBy.isEmpty()) {
                if (like.isLiked) {
                    if (like.count == 1) {
                        likedText.text =
                            context.getString(commonR.string.feed_component_liked_count_text_only_me)
                    } else {
                        likedText.text =
                            MethodChecker.fromHtml(
                                context.getString(
                                    commonR.string.feed_component_liked_by_text_me,
                    ...
                    likedText.text =
                        MethodChecker.fromHtml(
                            context.getString(
                                commonR.string.feed_component_liked_count_text,
                ...
                likedText.text = MethodChecker.fromHtml(
                    context.getString(
                        commonR.string.feed_component_liked_by_text,
    ...
    private fun bindComment(
        ...
        addCommentHint.hint = context.getString(commonR.string.feed_component_add_comment, name)
    ...
    private fun setVODView(
        ...
        feedVODViewHolder.setListener(
            listener = object : FeedVODViewHolder.VODListener {
    ...
    private fun setNewASGCLayout(feedXCard: FeedXCard) {
        ...
        pageControl.apply {
            setIndicator(
                if (totalProducts <= MAX_PRODUCT_TO_SHOW_IN_ASGC_CAROUSEL) {
                    totalProducts
                } else {
                    MAX_PRODUCT_TO_SHOW_IN_ASGC_CAROUSEL
                }
            )
            setCurrentIndicator(feedXCard.lastCarouselIndex)
    ...
    fun setCommentCount(comments: FeedXComments) {
        ...
        seeAllCommentText.text =
            context.getString(commonR.string.feed_component_see_all_comments, comments.countFmt)
    }
        ...
        topAdsJob = scope.launch {
            ...
            if (card.isTypeProductHighlight) {
                if ((card.campaign.isRilisanSpl || card.campaign.isFlashSaleToko) && colorGradient.isNotEmpty()) {
                    changeCTABtnColorAsPerColorGradientFromBE(
                        colorGradient.map { colorGradient ->
                            colorGradient.color
                        } as? ArrayList<String>
                    )
    ...
    private fun getCTAButtonText(card: FeedXCard) =
        if (card.isTypeProductHighlight) {
            ...
            context.getString(commonR.string.feeds_cek_sekarang)
        }
