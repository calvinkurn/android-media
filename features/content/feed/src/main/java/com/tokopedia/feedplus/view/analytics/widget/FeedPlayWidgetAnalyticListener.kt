...
class FeedPlayWidgetAnalyticListener @Inject constructor(
    ...
    override fun onClickViewAll(view: PlayWidgetSmallView, widgetPosition: Int) {
        TrackApp.getInstance().gtm.sendGeneralEvent(
                mapOf(
                        KEY_EVENT to "clickFeed",
                        KEY_EVENT_CATEGORY to "feed updates - cmp",
                        KEY_EVENT_ACTION to "click view all",
                        KEY_EVENT_LABEL to "0 - Tokopedia Play - $widgetPosition",
                        KEY_BUSINESS_UNIT to KEY_TRACK_BUSINESS_UNIT,
                        KEY_CURRENT_SITE to KEY_TRACK_CURRENT_SITE,
                        KEY_USER_ID to userId
                )
    ...
    override fun onClickChannelCard(view: PlayWidgetSmallView, item: PlayWidgetSmallChannelUiModel, channelPositionInList: Int, isAutoPlay: Boolean, widgetPosition: Int) {
        val trackerMap = BaseTrackerBuilder().constructBasicPromotionClick(
                event = "promoClick",
                eventCategory = "feed updates - cmp",
                eventAction = "click",
                eventLabel = "click channel - ${item.channelType.toTrackingType()} - ${item.channelId} - $channelPositionInList - $widgetPosition - is autoplay $isAutoPlay",
                promotions = listOf(
                        BaseTrackerConst.Promotion(
                                id = item.channelId,
                                name = "feed - play sgc channel - $widgetPosition",
                                creative = item.title,
    ...
    override fun onClickBannerCard(view: PlayWidgetSmallView, widgetPosition: Int) {
        TrackApp.getInstance().gtm.sendGeneralEvent(
                mapOf(
                        KEY_EVENT to "clickFeed",
                        KEY_EVENT_CATEGORY to "feed updates - cmp",
                        KEY_EVENT_ACTION to "click other content",
                        KEY_EVENT_LABEL to widgetPosition,
                        KEY_BUSINESS_UNIT to KEY_TRACK_BUSINESS_UNIT,
                        KEY_CURRENT_SITE to KEY_TRACK_CURRENT_SITE,
                        KEY_USER_ID to userId
                )
    ...
    override fun onImpressChannelCard(view: PlayWidgetSmallView, item: PlayWidgetSmallChannelUiModel, channelPositionInList: Int, isAutoPlay: Boolean, widgetPosition: Int) {
        val trackerMap = BaseTrackerBuilder().constructBasicPromotionView(
                event = "promoView",
                eventCategory = "feed updates - cmp",
                eventAction = "impression on play sgc channel",
                eventLabel = "${item.channelType.toTrackingType()} - ${item.channelId} - $channelPositionInList - $widgetPosition - is autoplay $isAutoPlay",
                promotions = listOf(
                        BaseTrackerConst.Promotion(
                                id = item.channelId,
                                name = "feed - play sgc channel - $widgetPosition",
