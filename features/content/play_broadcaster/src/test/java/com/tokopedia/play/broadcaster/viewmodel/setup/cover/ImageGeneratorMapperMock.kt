...
class ImageGeneratorMapperMock(
    ...
    fun getImagePreview(isUnselected: Boolean = false, channelTitle: String, channelAuthor: String, selectedPosition: Int): List<AutoGeneratedCoverUIModel> {
        val data = mutableListOf<AutoGeneratedCoverUIModel>()
        CoverColor.values.forEachIndexed { index, coverColor ->
            data.add(AutoGeneratedCoverUIModel(imageUrl = generateImage(coverColor.colorValue),
                channelAuthor = channelAuthor,
                channelTitle = channelTitle,
                isChecked = if (isUnselected) false else index == selectedPosition))
        }
        return data
    }
