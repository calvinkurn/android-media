...
class ImageGeneratorMapper(
    private val productSectionList: List<ProductTagSectionUiModel> = emptyList(),
    private val authorLogo: String = "",
) {

    // expected to be hardcoded with this order
    // ("blue", "red", "turquoise", "purple", "orange", "green")
    val backgroundList = listOf("blue", "red", "turquoise", "purple", "orange", "green")
    ...
    fun getImagePreview(): List<AutoGeneratedCoverUIModel> {
        ...
        backgroundList.forEach { backgroundColor ->
            data.add(AutoGeneratedCoverUIModel(imageUrl = generateImage(backgroundColor)))
        }
        return data
    }

    fun getCoverID(imageUrl: String): String {
        return imageUrl.substringAfter("$SOURCE_ID/")
    }
