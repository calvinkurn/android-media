...
class HydraSharedPreferences @Inject constructor(
    ...
    fun setLiveToVodBottomSheetPref(authorId: String) {
        mSharedPrefs.edit()
            .putBoolean(
                String.format(KEY_LIVE_TO_VOD_BOTTOM_SHEET, authorId),
                false,
            ).apply()
    }

    fun getLiveToVodBottomSheetPref(authorId: String): Boolean {
        return mSharedPrefs.getBoolean(
            String.format(KEY_LIVE_TO_VOD_BOTTOM_SHEET, authorId),
            true,
        )
    ...
    fun setLiveToVodTickerPref(authorId: String) {
        mSharedPrefs.edit()
            .putBoolean(
                String.format(KEY_LIVE_TO_VOD_TICKER, authorId),
                false,
            ).apply()
    }

    fun getLiveToVodTickerPref(authorId: String): Boolean {
        return mSharedPrefs.getBoolean(
            String.format(KEY_LIVE_TO_VOD_TICKER, authorId),
            true,
        )
    ...
    companion object {
        ...
        private const val KEY_LIVE_TO_VOD_BOTTOM_SHEET = "live_to_vod_bottom_sheet_%s"
        private const val KEY_LIVE_TO_VOD_TICKER = "live_to_vod_ticker_%s"
    }
}
