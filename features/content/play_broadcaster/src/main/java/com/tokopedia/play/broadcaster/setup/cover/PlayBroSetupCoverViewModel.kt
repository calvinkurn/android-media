...
class PlayBroSetupCoverViewModel @AssistedInject constructor(
    ...
    @Assisted(KEY_PRODUCT_SECTION_LIST) val productSectionList: List<ProductTagSectionUiModel>,
    ...
    interface Factory {
        fun create(
            ...
            @Assisted(KEY_PRODUCT_SECTION_LIST) productSectionList: List<ProductTagSectionUiModel>,
    ...
    val autoGeneratedCover = MutableStateFlow<List<AutoGeneratedCoverUIModel>>(emptyList())
    ...
    private val savedAutoGeneratedCoverPosition: Int
        get() = sharedPref.getSavedSelectedAutoGeneratedCover()
    ...
    fun submitAction(event: PlayBroSetupCoverAction) {
        when (event) {
            ...
            is PlayBroSetupCoverAction.SetUploadAutoGeneratedCover -> handleSaveSelectedImageCover()
    ...
    private fun checkHasPreselectedAutoGeneratedCover() {
        viewModelScope.launchCatchError(block = {
            _uiEvent.emit(
                PlayBroSetupCoverEvent.ButtonState(
    ...
    private fun handleSelectAutoGeneratedCover(data: AutoGeneratedCoverUIModel, position: Int) {
        viewModelScope.launchCatchError(block = {
    ...
    private fun handleSaveSelectedImageCover() {
        viewModelScope.launchCatchError(block = {
            saveAutoGeneratedCover(selectedAutoGeneratedCoverPosition)
            _uiEvent.emit(PlayBroSetupCoverEvent.SubmitCoverLoading(isLoading = true))
            ...
            uploadSelectedCover()
            _uiEvent.emit(
                PlayBroSetupCoverEvent.SubmitCoverLoading(isLoading = false)
            )
        }, onError = {
            _uiEvent.emit(
                PlayBroSetupCoverEvent.SubmitCoverLoading(
                    isLoading = false,
                    throwable = it,
                )
            )
    ...
    private fun saveAutoGeneratedCover(position: Int) {
        sharedPref.savedSelectedAutoGeneratedCover(position)
    }

    private fun deleteSavedAutoGeneratedCover() {
        sharedPref.savedSelectedAutoGeneratedCover(-1)
    }
    ...
    companion object {
        ...
        private const val KEY_PRODUCT_SECTION_LIST = "productSectionList"
