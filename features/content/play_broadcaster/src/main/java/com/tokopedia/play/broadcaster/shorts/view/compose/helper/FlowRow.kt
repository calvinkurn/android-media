package com.tokopedia.play.broadcaster.shorts.view.compose.helper
...
fun FlowRow(
    ...
    for (measurable in measurables) {
        ...
        if (lastRow != null && lastRow.width + hGapPx + placeable.width <= constraints.maxWidth) {
            ...
            lastRow.height = Integer.max(lastRow.height, placeable.height)
        } else {
    ...
    val height = rows.sumBy { row -> row.height } + Integer.max(vGapPx.times(rows.size - 1), 0)
