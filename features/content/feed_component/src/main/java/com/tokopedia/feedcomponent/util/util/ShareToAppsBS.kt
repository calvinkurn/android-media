...
class ShareToAppsBS : BottomSheetUnify(), ShareAdapter.OnItemClickListener {
...
//        }
        ...
        fun newInstance(listener: OnShareItemClickListener, name: String, avatar: String, link: String, shareFormat: String, shareTitle: String, mediaUrl: String? = null): ShareToAppsBS {
            val bottomSheet = ShareToAppsBS()
...
//    btnClose?.setOnClickListener { dismiss()
    ...
    private fun init() {
        val adapter = ShareAdapter(
                generateAvailableShareTypes(
                        mutableListOf<ShareBottomSheets.MimeType>().apply {
                            add(ShareBottomSheets.MimeType.TEXT)
                            if (arguments?.get(EXTRA_MEDIA_URL) != null) add(ShareBottomSheets.MimeType.IMAGE)
                        }
                )
        )
    ...
    private fun doActivityShare(type: ShareBottomSheets.ShareType.ActivityShare) {
        try {
    ...
    private fun doActionShare(type: ShareBottomSheets.ShareType.ActionShare) {
        type.handler()
    }
    ...
    private fun getTextIntent(packageName: String, className: String): Intent {
        return Intent(Intent.ACTION_SEND)
                .setType(ShareBottomSheets.MimeType.TEXT.typeString)
    ...
    private fun getInstagramStoryIntent(mediaUri: Uri): Intent {
        ...
        return Intent(ACTION_INSTAGRAM_STORY)
                .setType(ShareBottomSheets.MimeType.IMAGE.typeString)
    ...
    private fun generateAvailableShareTypes(typeList: List<ShareBottomSheets.MimeType>): List<ShareBottomSheets.ShareType> {
        return mutableListOf<ShareBottomSheets.ShareType>().apply {
            add(ShareBottomSheets.ShareType.ActivityShare(KEY_WHATSAPP, getString(R.string.share_whatsapp), ShareBottomSheets.MimeType.TEXT, getWhatsAppIntent(PACKAGE_NAME_WHATSAPP)))
            add(ShareBottomSheets.ShareType.ActivityShare(KEY_FACEBOOK, getString(R.string.share_facebook), ShareBottomSheets.MimeType.TEXT, getTextIntent(PACKAGE_NAME_FACEBOOK, CLASS_NAME_FACEBOOK)))
            add(ShareBottomSheets.ShareType.ActivityShare(KEY_LINE, getString(R.string.share_line).toUpperCase(), ShareBottomSheets.MimeType.TEXT, getTextIntent(PACKAGE_NAME_LINE, CLASS_NAME_LINE)))
            add(ShareBottomSheets.ShareType.ActivityShare(KEY_TWITTER, getString(R.string.share_twitter), ShareBottomSheets.MimeType.TEXT, getTextIntent(PACKAGE_NAME_TWITTER, CLASS_NAME_TWITTER)))

            val mediaUrl: String? = arguments?.getString(EXTRA_MEDIA_URL)
            if (typeList.contains(ShareBottomSheets.MimeType.IMAGE) && mediaUrl?.isNotEmpty() == true) {
                add(ShareBottomSheets.ShareType.ActivityShare(KEY_INSTAGRAM_FEED, getString(R.string.share_instagram_feed), ShareBottomSheets.MimeType.IMAGE, getInstagramFeedIntent(Uri.parse(mediaUrl))))
                add(ShareBottomSheets.ShareType.ActivityShare(KEY_INSTAGRAM_STORY, getString(R.string.share_instagram_story), ShareBottomSheets.MimeType.IMAGE, getInstagramStoryIntent(Uri.parse(mediaUrl))))
            }

            add(ShareBottomSheets.ShareType.ActionShare(KEY_COPY, getString(R.string.copy), ShareBottomSheets.MimeType.TEXT, R.drawable.ic_copy_clipboard, ::actionCopy))
            add(ShareBottomSheets.ShareType.ActionShare(KEY_OTHER, getString(R.string.other), ShareBottomSheets.MimeType.TEXT, R.drawable.ic_btn_more, ::actionMore))
        }
                .filterNot { shareType -> shareType is ShareBottomSheets.ShareType.ActivityShare && shareType.getResolveActivity(context as Context) == null }
                .distinctBy(ShareBottomSheets.ShareType::key)
    }
    ...
    override fun onItemClick(type: ShareBottomSheets.ShareType) {
        when (type) {
            is ShareBottomSheets.ShareType.ActivityShare -> doActivityShare(type)
            is ShareBottomSheets.ShareType.ActionShare -> doActionShare(type)
        }
    }
}