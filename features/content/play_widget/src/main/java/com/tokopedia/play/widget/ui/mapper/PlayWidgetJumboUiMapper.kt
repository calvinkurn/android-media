 ...
 */
class PlayWidgetJumboUiMapper @Inject constructor(
    ...
    override fun mapWidget(data: PlayWidget, prevModel: PlayWidgetUiModel?): PlayWidgetUiModel {
        val prevJumboModel = prevModel as? PlayWidgetUiModel.Jumbo

        return PlayWidgetUiModel.Jumbo(
            ...
            items = mapWidgetItem(prevJumboModel?.items, data.data)
        )
    }

    private fun mapWidgetItem(
        prevItems: List<PlayWidgetJumboItemUiModel>?,
        items: List<PlayWidgetItem>
    ): List<PlayWidgetJumboItemUiModel> = items.mapNotNull {
        when (it.typename) {
            "banner" -> mapWidgetItemBanner(it)
            "channel" -> mapWidgetItemChannel(
                prevItems?.find { prevItem -> prevItem is PlayWidgetJumboChannelUiModel && prevItem.channelId == it.id } as? PlayWidgetJumboChannelUiModel,
                it
            )
            else -> null
        }
    ...
    private fun mapWidgetItemBanner(item: PlayWidgetItem): PlayWidgetJumboBannerUiModel =
        PlayWidgetJumboBannerUiModel(
    ...
    private fun mapWidgetItemChannel(
        prevItem: PlayWidgetJumboChannelUiModel?,
        item: PlayWidgetItem
    ): PlayWidgetJumboChannelUiModel {
        val channelType = PlayWidgetChannelType.getByValue(item.widgetType)

        return PlayWidgetJumboChannelUiModel(
            channelId = item.id,
            title = item.title,
