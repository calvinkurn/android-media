...
object Events {
    ...
    fun itemClick(view: AbsListView): Observable<Int> {
        ...
        view.onItemClickListener = AdapterView.OnItemClickListener { adapterView, view, position, l -> subject.onNext(position) }
        return subject
    }

    fun select(view: AutoCompleteTextView): PublishSubject<Int> {
        val subject = PublishSubject.create<Int>()
        view.onItemClickListener = AdapterView.OnItemClickListener { adapterView, view, i, l -> subject.onNext(i + 1) }
        return subject
    }
}