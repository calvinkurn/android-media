...
object DeeplinkDFMapper : CoroutineScope {
    ...
    private val deeplinkDFPatternListCustomerApp: List<DFP> by lazy {
        mutableListOf<DFP>().apply {
            ...
            add(DFP({ it.startsWithPattern(AFFILIATE_EXPLORE)}, DF_CONTENT_AFFILIATE, R.string.applink_title_affiliate_explore))
            add(DFP({ it.startsWithPattern(AFFILIATE_DASHBOARD)}, DF_CONTENT_AFFILIATE, R.string.applink_title_affiliate_dashboard))
            add(DFP({ it.startsWithPattern(TOKOPEDIA_BYME_HTTP) || it.startsWithPattern(TOKOPEDIA_BYME_HTTPS) }, DF_CONTENT_AFFILIATE, R.string.applink_title_affiliate))
