...
object DeeplinkMapperPromo {
    ...
    fun getDynamicDeeplinkForTokomember(deeplink: String): String {
        val uri = Uri.parse(deeplink)
        return when {
            deeplink.contains(ApplinkConst.Tokomember.PROGRAM_EXTENSION) -> getDeeplinkForProgramExtension(
                uri
            )
            else -> ""
        }
    ...
    fun getDeeplinkForProgramExtension(deeplink: Uri): String {
        if (UriUtil.matchWithPattern(
                ApplinkConst.SellerApp.TOKOMEMBER_PROGRAM_EXTENSION,
                deeplink
            ) != null
        ) {
            val programId = deeplink.lastPathSegment
            return UriUtil.buildUri(
                ApplinkConstInternalSellerapp.TOKOMEMBER_PROGRAM_EXTENSION,
                programId
            )
        }
        return ""
