...
object DeeplinkMapper {
    ...
    val deeplinkPatternTokopediaSchemeList: MutableList<DLP> = mutableListOf(
        ...
        DLP.matchPattern(ApplinkConst.PROFILE) { _, _, deeplink, _ -> DeeplinkMapperContent.getProfileDeeplink(deeplink) },
        ...
        DLP.startWith(ApplinkConst.SALAM_UMRAH) { ctx, _, deeplink, _ -> getRegisteredNavigationSalamUmrah(deeplink) },
        DLP.startWith(ApplinkConst.SALAM_UMRAH_ORDER_DETAIL) { ctx, _, deeplink, _ -> getRegisteredNavigationSalamUmrahOrderDetail(deeplink) },
        ...
        DLP.startWith(ApplinkConst.POWER_MERCHANT_SUBSCRIBE) { ctx, uri, _, _ -> PowerMerchantDeepLinkMapper.getPowerMerchantAppLink(ctx, uri) },
        ...
        DLP.exact(ApplinkConst.MERCHANT_VOUCHER_LIST, ApplinkConstInternalSellerapp.SELLER_MVC_LIST_ACTIVE),
        ...
        DLP.exact(ApplinkConst.TokopediaNow.HOME) { _, _, deeplink, _ -> getRegisteredNavigationTokopediaNowHome(deeplink) },
        ...
        DLP.startWith(ApplinkConst.TokopediaNow.OLD_CATEGORY) { _, _, deeplink, _ -> getRegisteredNavigationTokopediaNowCategory(deeplink) },
        ...
        DLP.matchPattern(ApplinkConst.GOTO_KYC) { ctx, _, deeplink, _ -> DeeplinkMapperUser.getRegisteredNavigationUser(ctx, deeplink) }
    ).apply {
    ...
    private fun getRegisteredNavigationFromInternalTokopedia(context: Context, uri: Uri, deeplink: String): String {
        return when {
            ...
            deeplink.startsWithPattern(ApplinkConstInternalUserPlatform.GOTO_KYC) -> DeeplinkMapperUser.getRegisteredNavigationUser(context, deeplink)
            else -> return ""
        }
    }
    ...
    private fun getRegisteredNavigationFromSellerapp(context: Context, uri: Uri, deeplink: String): String {
        return when (val trimmedDeeplink = UriUtil.trimDeeplink(uri, deeplink)) {
            ...
            ApplinkConst.SellerApp.POWER_MERCHANT_SUBSCRIBE -> PowerMerchantDeepLinkMapper.getPowerMerchantAppLink(context, uri)
