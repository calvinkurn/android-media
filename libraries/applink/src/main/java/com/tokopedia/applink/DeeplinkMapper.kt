...
object DeeplinkMapper {
    ...
    val deeplinkPatternTokopediaSchemeList: MutableList<DLP> = mutableListOf(
        ...
        DLP.host(ApplinkConst.INBOX_HOST) { _, _, deeplink, _ ->
            DeeplinkMapperHome.getRegisteredInboxNavigation(
                deeplink
            )
        },
        ...
        DLP.exact(ApplinkConst.TRAVEL_SUBHOMEPAGE_HOME) { ctx, _, deeplink, _ ->
        ...
        DLP.matchPattern(ApplinkConst.SHOP_OPERATIONAL_HOUR) { _, uri, _, idList ->
            DeeplinkMapperMarketplace.getShopOperationalHourInternalAppLink(
                idList?.getOrNull(0).orEmpty()
            )
        },
        DLP.matchPattern(ApplinkConst.SHOP_MVC_LOCKED_TO_PRODUCT) { _, uri, _, idList ->
            ...
            DeeplinkMapperMarketplace.getShopMvcLockedToProductShopIdInternalAppLink(
                shopId,
                voucherId
            )
        },
        ...
        DLP.startWith(ApplinkConst.POWER_MERCHANT_SUBSCRIBE) { ctx, _, _, _ ->
        ...
        DLP.startWith(ApplinkConst.TokoFood.MAIN_PATH) { _, uri, _, _ ->
            DeeplinkMapperTokoFood.mapperInternalApplinkTokoFood(
                uri
            )
        },
        DLP.startWith(ApplinkConst.TokoFood.GOFOOD) { _, uri, _, _ ->
            DeeplinkMapperTokoFood.mapperInternalApplinkTokoFood(
                uri
            )
        },
...
class Exact(target: String) :
    DLPLogic(logic = { _, _, deeplink -> ((deeplink == target) to null) }, true)
