...
class DeepLinkDFMapperCustomerAppTest : DeepLinkDFMapperTestFixture() {
    ...
    @Test
    fun `check merchant shop score appLink then should return DF_MERCHANT_SELLER in customerapp`() {
        val appLink = "${ApplinkConstInternalMechant.INTERNAL_MERCHANT}/shop-score-detail"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_MERCHANT_SELLER)
    }

    @Test
    fun `check shop score detail appLink then should return DF_MERCHANT_SELLER in customerapp`() {
        val appLink = "${DeeplinkConstant.SCHEME_TOKOPEDIA}://shop-score-detail"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_MERCHANT_SELLER)
    }

    @Test
    fun `check shop score detail appLink with sellerapp scheme then should return DF_MERCHANT_SELLER in customerapp`() {
    ...
    @Test
    fun `check shop internal appLink then should return DF_BASE in customerapp`() {
        val appLink = "${ApplinkConstInternalMarketplace.INTERNAL_MARKETPLACE}/shop-page/12345"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_BASE)
    }
    ...
    @Test
    fun `check shop settings internal appLink then should return DF_MERCHANT_SELLER in customerapp`() {
        val internalAppLink = "${ApplinkConstInternalMarketplace.INTERNAL_MARKETPLACE}/shop-page/12345/settings"
        assertEqualDeepLinkCustomerApp(internalAppLink, DeeplinkDFMapper.DF_MERCHANT_SELLER)
    }
    ...
    @Test
    fun `check shop operational hour bottom sheet appLink then should return DF_BASE in customerapp`() {
    ...
    @Test
    fun `check new add phone appLink then should return DF_USER_SETTINGS in customerapp`() {
        val appLink = "${ApplinkConstInternalUserPlatform.NEW_INTERNAL_USER}/new-add-phone"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_USER_SETTINGS)
    }
    ...
    @Test
    fun `check goto kyc webview from applink then should return DF_USER_SETTINGS in customerapp`() {
        val appLink = "${ApplinkConstInternalUserPlatform.NEW_INTERNAL_USER}/webview-kyc?url=tokopedia.com"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_USER_SETTINGS)
    }
    ...
    fun `check liveness detection appLink then should return DF_USER_LIVENESS in customerapp`() {
        val appLink = "${ApplinkConstInternalUserPlatform.NEW_INTERNAL_USER}/liveness-detection?projectId=0"
    ...
    @Test
    fun `check tokopedia now home appLink then should return DF_TOKOPEDIA_NOW in customerapp`() {
        val appLink = "${DeeplinkConstant.SCHEME_INTERNAL}://now/home"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_TOKOPEDIA_NOW)
    }

    @Test
    fun `check tokopedia now category appLink then should return DF_TOKOPEDIA_NOW in customerapp`() {
        val appLink = "${DeeplinkConstant.SCHEME_INTERNAL}://now/category"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_TOKOPEDIA_NOW)
    }

    @Test
    fun `check tokopedia now search appLink then should return DF_TOKOPEDIA_NOW in customerapp`() {
    ...
    fun `check shop review appLink then should return DF_MERCHANT_REVIEW in customerapp`() {
        ...
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_MERCHANT_NONLOGIN)
    }
    ...
    @Test
    fun `check old product review appLink then should return DF_MERCHANT_REVIEW in customerapp`() {
        val appLink = "${ApplinkConstInternalMarketplace.INTERNAL_MARKETPLACE}/product/123/review-old"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_MERCHANT_NONLOGIN)
    }

    @Test
    fun `check inbox reputation appLink then should return DF_MERCHANT_REVIEW in customerapp`() {
        val appLink = "${ApplinkConstInternalMarketplace.INTERNAL_MARKETPLACE}/review"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_MERCHANT_NONLOGIN)
    }

    @Test
    fun `check create review appLink then should return DF_MERCHANT_REVIEW in customerapp`() {
        val appLink = "${ApplinkConstInternalMarketplace.INTERNAL_MARKETPLACE}/product-review/create/123/456/"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_MERCHANT_NONLOGIN)
    }

    @Test
    fun `check review detail appLink then should return DF_MERCHANT_REVIEW in customerapp`() {
        val appLink = "${ApplinkConstInternalMarketplace.INTERNAL_MARKETPLACE}/review/detail/123/"
        assertEqualDeepLinkCustomerApp(appLink, DeeplinkDFMapper.DF_MERCHANT_NONLOGIN)
    }
    ...
    @Test
    fun `check affiliate appLink then should return DF_CATEGORY_AFFILIATE in customerapp`() {
        val internalApplink = "${DeeplinkConstant.SCHEME_INTERNAL}://affiliate"
        assertEqualDeepLinkCustomerApp(internalApplink, DeeplinkDFMapper.DF_CATEGORY_AFFILIATE)
    }
}
