...
public class XRenderer extends AxisRenderer {
	...
	private XRendererListener xRendererListener = new XRendererListener() {
		@Override
		public boolean filterX(@IntRange(from = 0) int position) {
			return true;
		}
	};
	...
	public void setxDataGrid(boolean xDataGrid) {
		this.xDataGrid = xDataGrid;
	}

	public void setxRendererListener(XRendererListener xRendererListener) {
	 ...
	 *
	 * @param canvas {@link Canvas} object to use while drawing the data.
	 */
	@Override
	public void draw(Canvas canvas) {
		...
		// Draw axis
		if (style.hasXAxis())
			canvas.drawLine(mInnerChartLeft, axisPosition, rightCanvas, axisPosition,
					  style.getChartPaint());
		...
		if (labelsPositioning != LabelPosition.NONE) {
			...
			for (int i = 0; i < nLabels; i++) {
				if(xRendererListener != null && xRendererListener.filterX(i)) {
					String formatedLabel = stringFormatRenderer != null ? stringFormatRenderer.formatString(labels.get(i)) : labels.get(i);
					canvas.drawText(formatedLabel, labelsPos.get(i), labelsStaticPos,
	...
	/**
	 * define some value display or not
	 */
	public interface XRendererListener{

		/**
		 * filter x labels - prevent display
		 * @param position valid position from 0 to max labels
         * @return true if x axis label should be displayed, false otherwise
         */
		boolean filterX(@IntRange(from=0) int position);
	}
