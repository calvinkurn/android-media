...
//		pathline.quadTo(controlX, controlY, pointX, pointY);
	...
	public void onDrawChart(Canvas canvas, ArrayList<ChartSet> data) {
		...
		for (ChartSet set : data) {
			...
			if (lineSet.isVisible()) {
				...
				if (lineSet.isSmooth()==LineSet.NON_SMOOTH) linePath = createLinePath(lineSet);
	...
	private Path createSmoothQuadLinePath(LineSet set) {
		float thisPointX;
		float thisPointY;
		float nextPointX;
		float nextPointY;
		...
		for (int i = begin; i < end - 1; i++) {
			...
			prePoint.set(thisPointX, thisPointY);

			nextPointX = set.getEntry(i + 1).getX();
			nextPointY = set.getEntry(i + 1).getY();
			nextPointF.set(nextPointX, nextPointY);

			pathCubicTo(res, prePoint, nextPointF);
		}
