...
public class CropperGridView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray mTypedArray = context.obtainStyledAttributes(attrs, R.styleable.CropperGridView);
            mColor = mTypedArray.getColor(R.styleable.CropperGridView_nocropper__grid_color, mColor);

            float alpha = 255 * mTypedArray.getFloat(R.styleable.CropperGridView_nocropper__grid_opacity, 1f);
            if (alpha < 0) {
                alpha = 0;
            } else if (alpha > 255) {
                alpha = 255;
            ...
            mStrokeWidth = mTypedArray.getDimensionPixelOffset(R.styleable.CropperGridView_nocropper__grid_thickness,
                    mStrokeWidth);
