...
interface WebSocketLogDao {
    ...
    @Query("SELECT * FROM websocket_log WHERE channel_id LIKE :query OR gc_token lIKE :query OR event LIKE :query OR message LIKE :query ORDER BY timestamp DESC LIMIT :limit OFFSET :offset")
    fun get(query: String, limit: Int, offset: Int): List<WebSocketLogEntity>
}