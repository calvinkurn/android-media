package com.tokopedia.graphql.util

object Const {
    const val GQL_RESPONSE_HTTP_OK = 200
    const val GQL_ERROR_MAX_LENGTH = 1000
    const val GQL_RESPONSE_MAX_LENGTH = 100
    const val GQL_SHORTENED_ERROR_LOG_MAX_LENGTH = 200
    const val GQL_REQUEST_LOG_MAX_LENGTH = 1000
    const val GQL_LONG_RESPONSE_LOG_MAX_LENGTH = 2000

    const val GQL_QUERY_HASHING_ERROR = 244
    const val AKAMAI_SENSOR_DATA_HEADER = "X-acf-sensor-data"
    const val TKPD_AKAMAI = "X-TKPD-AKAMAI"
    const val QUERY_HASHING_HEADER = "queryhash"
}
