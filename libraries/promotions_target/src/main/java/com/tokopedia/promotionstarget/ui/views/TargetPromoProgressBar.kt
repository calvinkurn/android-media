...
class TargetPromoProgressBar(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    var progressBarWidth: Float = resources.getDimension(R.dimen.tpromo_default_stroke_width)
        set(value) {
            field = value.dpToPx()
    ...
    var backgroundProgressBarWidth: Float = resources.getDimension(R.dimen.tpromo_default_background_stroke_width)
        set(value) {
            field = value.dpToPx()
    ...
    private val indeterminateModeRunnable = Runnable {
        if (indeterminateMode) {
            ...
            this@TargetPromoProgressBar.progressDirectionIndeterminateMode = this@TargetPromoProgressBar.progressDirectionIndeterminateMode.reverse()
            if (this@TargetPromoProgressBar.progressDirectionIndeterminateMode.isToRight()) {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        val attributes = context.theme.obtainStyledAttributes(attrs, R.styleable.TargetPromoProgressBar, 0, 0)

        // Value
        progress = attributes.getFloat(R.styleable.TargetPromoProgressBar_tpromo_progress, progress)
        progressMax = attributes.getFloat(R.styleable.TargetPromoProgressBar_tpromo_progress_max, progressMax)

        // StrokeWidth
        progressBarWidth = attributes.getDimension(R.styleable.TargetPromoProgressBar_tpromo_progressbar_width, progressBarWidth).pxToDp()
        backgroundProgressBarWidth = attributes.getDimension(R.styleable.TargetPromoProgressBar_tpromo_background_progressbar_width, backgroundProgressBarWidth).pxToDp()

        // Color
        progressBarColor = attributes.getInt(R.styleable.TargetPromoProgressBar_tpromo_progressbar_color, progressBarColor)
        attributes.getColor(R.styleable.TargetPromoProgressBar_tpromo_progressbar_color_start, 0)
                .also { if (it != 0) progressBarColorStart = it }
        attributes.getColor(R.styleable.TargetPromoProgressBar_tpromo_progressbar_color_end, 0)
                .also { if (it != 0) progressBarColorEnd = it }
        progressBarColorDirection = attributes.getInteger(R.styleable.TargetPromoProgressBar_tpromo_progressbar_color_direction, progressBarColorDirection.value).toGradientDirection()
        backgroundProgressBarColor = attributes.getInt(R.styleable.TargetPromoProgressBar_tpromo_background_progressbar_color, backgroundProgressBarColor)
        attributes.getColor(R.styleable.TargetPromoProgressBar_tpromo_background_progressbar_color_start, 0)
                .also { if (it != 0) backgroundProgressBarColorStart = it }
        attributes.getColor(R.styleable.TargetPromoProgressBar_tpromo_background_progressbar_color_end, 0)
                .also { if (it != 0) backgroundProgressBarColorEnd = it }
        backgroundProgressBarColorDirection = attributes.getInteger(R.styleable.TargetPromoProgressBar_tpromo_background_progressbar_color_direction, backgroundProgressBarColorDirection.value).toGradientDirection()

        // Progress Direction
        progressDirection = attributes.getInteger(R.styleable.TargetPromoProgressBar_tpromo_progress_direction, progressDirection.value).toProgressDirection()

        // Round Border
        roundBorder = attributes.getBoolean(R.styleable.TargetPromoProgressBar_tpromo_round_border, roundBorder)

        // Angle
        startAngle = attributes.getFloat(R.styleable.TargetPromoProgressBar_tpromo_start_angle, 0f)

        // Indeterminate Mode
        indeterminateMode = attributes.getBoolean(R.styleable.TargetPromoProgressBar_tpromo_indeterminate_mode, indeterminateMode)
