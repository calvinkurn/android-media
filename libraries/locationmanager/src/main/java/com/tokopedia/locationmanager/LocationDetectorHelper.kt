...
class LocationDetectorHelper(ctx: Context) {
    ...
    private fun getDataFromLocal(): DeviceLocation {
        ...
        return if (location == null) {
            ...
            if (latLegacy == DEFAULT_LATITUDE.toString() && lonLegacy == DEFAULT_LONGITUDE.toString()) {
                ...
                val locationFromLegacy = DeviceLocation(latLegacy.toDouble(), lonLegacy.toDouble(), Date().time)
    ...
    private fun getDataFromCloud(
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            var grantedCount = 0
            var totalCount = 0
            getPermissions(requestLocationType).onEach {
    ...
    private fun onSuccessGetLocation(location: Location, onGetLocation: (DeviceLocation) -> Unit) {
