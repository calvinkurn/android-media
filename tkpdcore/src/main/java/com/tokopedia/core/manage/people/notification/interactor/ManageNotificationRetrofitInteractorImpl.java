 ...
 */
public class ManageNotificationRetrofitInteractorImpl implements ManageNotificationRetrofitInteractor {

    private static final String TAG = ManageNotificationRetrofitInteractorImpl.class.getSimpleName();

    private final CompositeSubscription compositeSubscription;
    private final PeopleService peopleService;
    private final PeopleActService peopleActService;

    public ManageNotificationRetrofitInteractorImpl() {
    ...
    @Override
    public void getNotificationSetting(@NonNull final Context context,
                                       @NonNull Map<String, String> params,
                                       @NonNull final NotificationSettingListener listener) {
        Observable<Response<TkpdResponse>> observable = peopleService.getApi()
                .getNotification(AuthUtil.generateParams(context, params));
            ...
            public void onNext(Response<TkpdResponse> response) {
                if (response.isSuccessful()) {
                    if (!response.body().isError()) {
                        listener.onSuccess(response.body().convertDataObj(SettingNotification.class));
                    } else {
    ...
    @Override
    public void setNotificationSetting(@NonNull Context context,
                                       @NonNull Map<String, String> params,
                                       @NonNull final SetNotificationSettingListener listener) {
        Observable<Response<TkpdResponse>> observable = peopleActService.getApi()
                .editNotification(AuthUtil.generateParams(context, params));
