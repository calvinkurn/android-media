
package com.tokopedia.inbox.rescenter.discussion.data.pojo.replyvalidation;

import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;

public class SolutionLast {

    @SerializedName("last_trouble_type")
    @Expose
    private int lastTroubleType;
    @SerializedName("last_create_time")
    @Expose
    private String lastCreateTime;
    @SerializedName("last_show_accept_retur_button")
    @Expose
    private int lastShowAcceptReturButton;
    @SerializedName("last_show_input_resi_button")
    @Expose
    private int lastShowInputResiButton;
    @SerializedName("last_show_button_string_text")
    @Expose
    private String lastShowButtonStringText;
    @SerializedName("last_resolution_id")
    @Expose
    private int lastResolutionId;
    @SerializedName("last_show_accept_admin_button")
    @Expose
    private int lastShowAcceptAdminButton;
    @SerializedName("last_create_time_wib")
    @Expose
    private String lastCreateTimeWib;
    @SerializedName("last_show_accept_admin_retur_button")
    @Expose
    private int lastShowAcceptAdminReturButton;
    @SerializedName("last_show_accept_button")
    @Expose
    private int lastShowAcceptButton;
    @SerializedName("last_user_url")
    @Expose
    private String lastUserUrl;
    @SerializedName("last_show_appeal_button")
    @Expose
    private int lastShowAppealButton;
    @SerializedName("last_user_name")
    @Expose
    private String lastUserName;
    @SerializedName("last_action_by")
    @Expose
    private int lastActionBy;
    @SerializedName("last_refund_amt_idr")
    @Expose
    private String lastRefundAmtIdr;
    @SerializedName("last_refund_amt")
    @Expose
    private String lastRefundAmt;
    @SerializedName("last_flag_received")
    @Expose
    private int lastFlagReceived;
    @SerializedName("last_show_finish_button")
    @Expose
    private int lastShowFinishButton;
    @SerializedName("last_solution")
    @Expose
    private int lastSolution;

    public int getLastTroubleType() {
        return lastTroubleType;
    }

    public void setLastTroubleType(int lastTroubleType) {
        this.lastTroubleType = lastTroubleType;
    }

    public String getLastCreateTime() {
        return lastCreateTime;
    }

    public void setLastCreateTime(String lastCreateTime) {
        this.lastCreateTime = lastCreateTime;
    }

    public int getLastShowAcceptReturButton() {
        return lastShowAcceptReturButton;
    }

    public void setLastShowAcceptReturButton(int lastShowAcceptReturButton) {
        this.lastShowAcceptReturButton = lastShowAcceptReturButton;
    }

    public int getLastShowInputResiButton() {
        return lastShowInputResiButton;
    }

    public void setLastShowInputResiButton(int lastShowInputResiButton) {
        this.lastShowInputResiButton = lastShowInputResiButton;
    }

    public String getLastShowButtonStringText() {
        return lastShowButtonStringText;
    }

    public void setLastShowButtonStringText(String lastShowButtonStringText) {
        this.lastShowButtonStringText = lastShowButtonStringText;
    }

    public int getLastResolutionId() {
        return lastResolutionId;
    }

    public void setLastResolutionId(int lastResolutionId) {
        this.lastResolutionId = lastResolutionId;
    }

    public int getLastShowAcceptAdminButton() {
        return lastShowAcceptAdminButton;
    }

    public void setLastShowAcceptAdminButton(int lastShowAcceptAdminButton) {
        this.lastShowAcceptAdminButton = lastShowAcceptAdminButton;
    }

    public String getLastCreateTimeWib() {
        return lastCreateTimeWib;
    }

    public void setLastCreateTimeWib(String lastCreateTimeWib) {
        this.lastCreateTimeWib = lastCreateTimeWib;
    }

    public int getLastShowAcceptAdminReturButton() {
        return lastShowAcceptAdminReturButton;
    }

    public void setLastShowAcceptAdminReturButton(int lastShowAcceptAdminReturButton) {
        this.lastShowAcceptAdminReturButton = lastShowAcceptAdminReturButton;
    }

    public int getLastShowAcceptButton() {
        return lastShowAcceptButton;
    }

    public void setLastShowAcceptButton(int lastShowAcceptButton) {
        this.lastShowAcceptButton = lastShowAcceptButton;
    }

    public String getLastUserUrl() {
        return lastUserUrl;
    }

    public void setLastUserUrl(String lastUserUrl) {
        this.lastUserUrl = lastUserUrl;
    }

    public int getLastShowAppealButton() {
        return lastShowAppealButton;
    }

    public void setLastShowAppealButton(int lastShowAppealButton) {
        this.lastShowAppealButton = lastShowAppealButton;
    }

    public String getLastUserName() {
        return lastUserName;
    }

    public void setLastUserName(String lastUserName) {
        this.lastUserName = lastUserName;
    }

    public int getLastActionBy() {
        return lastActionBy;
    }

    public void setLastActionBy(int lastActionBy) {
        this.lastActionBy = lastActionBy;
    }

    public String getLastRefundAmtIdr() {
        return lastRefundAmtIdr;
    }

    public void setLastRefundAmtIdr(String lastRefundAmtIdr) {
        this.lastRefundAmtIdr = lastRefundAmtIdr;
    }

    public String getLastRefundAmt() {
        return lastRefundAmt;
    }

    public void setLastRefundAmt(String lastRefundAmt) {
        this.lastRefundAmt = lastRefundAmt;
    }

    public int getLastFlagReceived() {
        return lastFlagReceived;
    }

    public void setLastFlagReceived(int lastFlagReceived) {
        this.lastFlagReceived = lastFlagReceived;
    }

    public int getLastShowFinishButton() {
        return lastShowFinishButton;
    }

    public void setLastShowFinishButton(int lastShowFinishButton) {
        this.lastShowFinishButton = lastShowFinishButton;
    }

    public int getLastSolution() {
        return lastSolution;
    }

    public void setLastSolution(int lastSolution) {
        this.lastSolution = lastSolution;
    }

}
