...
import com.tokopedia.inbox.inboxmessageold.interactor.SendMessageRetrofitInteractor;
import com.tokopedia.inbox.inboxmessageold.interactor.SendMessageRetrofitInteractorImpl;
import com.tokopedia.inbox.inboxmessageold.model.SendMessagePass;
...
public class SendMessagePresenterImpl implements SendMessagePresenter {
    ...
    public boolean isValidMessage() {
        ...
        if (viewListener.getContent().trim().length() == 0) {
            isValid = false;
            viewListener.setContentError(viewListener.getString(com.tokopedia.core.R.string.error_field_required));
        }

        if (viewListener.getSubject().trim().length() == 0) {
            isValid = false;
            viewListener.setSubjectError(viewListener.getString(com.tokopedia.core.R.string.error_field_required));
        }
        ...
        if (isValidMessage()) {
            ...
            networkInteractor.sendMessage(viewListener.getActivity(), getSendMessageParam(), new SendMessageRetrofitInteractor.SendMessageListener() {
                ...
                public void onSuccess() {
                    ...
                    CommonUtils.UniversalToast(viewListener.getActivity(), viewListener.getString(com.tokopedia.core.R.string.success_send_msg));
                ...
                public void onTimeout() {
                    viewListener.setActionsEnabled(true);
                    viewListener.showError(viewListener.getString(com.tokopedia.core.R.string.msg_connection_timeout));
                }
                ...
                public void onNoNetworkConnection() {
                    viewListener.setActionsEnabled(true);
                    viewListener.showError(viewListener.getString(com.tokopedia.core.R.string.msg_no_connection));
                }
            });
        }
    }
