...
public class QuickReturnFooterBehavior extends CoordinatorLayout.Behavior<View> {
    ...
    public boolean onStartNestedScroll(CoordinatorLayout coordinatorLayout, View child, View directTargetChild, View target, int nestedScrollAxes) {
        ...
        if (layoutParams instanceof CoordinatorLayout.LayoutParams) {
            marginBottom = ((CoordinatorLayout.LayoutParams) layoutParams).bottomMargin;
        }
    ...
    protected ObjectAnimator getObjectAnimatorHideView(View child) {
        return ObjectAnimator.ofFloat(child, "translationY", 0, childHeight + marginBottom);
    }

    protected ObjectAnimator getObjectAnimatorShowView(View child) {
        return ObjectAnimator.ofFloat(child, "translationY", childHeight + marginBottom, 0);
    }
}