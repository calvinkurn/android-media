...
class ReadyToDeliverOrderUseCase @Inject constructor(
    private val readyToDeliverOrderRepository: ReadyToDeliverOrderRepository
) : OrderUseCase {

    override fun getOrderList(): Flow<List<OrderModel>> {
        return readyToDeliverOrderRepository.getCachedData().map {
            it.mapToDomainModel()
        }
    }

    override fun getCount(): Flow<Pair<String, Int>> {
        return readyToDeliverOrderRepository.getCachedDataCount().map {
