...
public class HomeFeedMapper implements Func1<HomeFeedQuery.Data, FeedDomain> {
    ...
    private ProductFeedDomain createProductFeedDomain(String cursor,
                                                      HomeFeedQuery.Data.Product product,
    ...
    private PromotionFeedDomain createPromotionFeedDomain(HomeFeedQuery.Data.Promotion promotion) {
    ...
    private WholesaleDomain createWholesaleDomain(HomeFeedQuery.Data.Wholesale wholesale) {
        return new WholesaleDomain(wholesale.qty_min_fmt());
    }

    private List<WholesaleDomain> convertToWholesaleDomain(List<HomeFeedQuery.Data.Wholesale> wholesales) {
    ...
    convertToProductFeedDomain(String cursor,
                               List<HomeFeedQuery.Data.Product> products) {
        List<ProductFeedDomain> productFeedDomains = new ArrayList<>();
        if (products != null) {
            for (int i = 0; i < products.size(); i++) {

                HomeFeedQuery.Data.Product product = products.get(i);
    ...
    convertToPromotionFeedDomain(List<HomeFeedQuery.Data.Promotion> promotions) {
    ...
    private ShopFeedDomain createShopFeedDomain(HomeFeedQuery.Data.Shop shop) {
    ...
    private SourceFeedDomain createSourceFeedDomain(
            HomeFeedQuery.Data.Source source, ShopFeedDomain shopFeedDomain) {
    ...
    private List<OfficialStoreDomain> convertToOfficialStoresFeedDomain(
            List<HomeFeedQuery.Data.Official_store> official_stores) {
        ...
        if (official_stores != null
            ...
            for (HomeFeedQuery.Data.Official_store officialStore : official_stores) {
    ...
    private List<OfficialStoreProductDomain> convertToOfficialStoreProducts(
            List<HomeFeedQuery.Data.Product1> products) {
        List<OfficialStoreProductDomain> listProduct = new ArrayList<>();
        if (products != null) {
            for (HomeFeedQuery.Data.Product1 product : products) {
    ...
    private DataDomain convertToOfficialStoreProductData(HomeFeedQuery.Data.Data1 data) {
    ...
    private List<LabelDomain> convertToOfficialStoreProductLabel(List<HomeFeedQuery.Data.Label> labels) {
        List<LabelDomain> listLabel = new ArrayList<>();
        if (labels != null) {
            for (HomeFeedQuery.Data.Label label : labels) {
    ...
    private List<BadgeDomain> convertToOfficialStoreProductBadge(List<HomeFeedQuery.Data.Badge> badges) {
        List<BadgeDomain> listBadge = new ArrayList<>();
        if (badges != null) {
            for (HomeFeedQuery.Data.Badge badge : badges) {
    ...
    private ShopDomain convertToOfficialStoreProductShopDomain(HomeFeedQuery.Data.Shop1 shop) {
    ...
    private KolRecommendationDomain convertToKolRecommendationDomain(HomeFeedQuery.Data.Kolrecommendation kolrecommendation) {
    ...
    private FavoriteCtaDomain convertToFavoriteCtaDomain(HomeFeedQuery.Data.Favorite_cta favoriteCta) {
        if (favoriteCta == null) {
            return null;
        } else {
    ...
    private List<KolRecommendationItemDomain> convertToListKolRecommendation(List<HomeFeedQuery.Data.Kol> kolrecommendation) {
        List<KolRecommendationItemDomain> list = new ArrayList<>();
        if (kolrecommendation != null) {
            for (HomeFeedQuery.Data.Kol recommendation : kolrecommendation) {
    ...
    private KolPostDomain createKolPostDomain(HomeFeedQuery.Data.Datum datum) {
        if (datum.content().kolpost() != null) {
            HomeFeedQuery.Data.Kolpost kolpost = datum.content()
                    .kolpost();
            HomeFeedQuery.Data.Content1 content = datum.content()
            ...
            HomeFeedQuery.Data.Followedkolpost kolpost = datum.content()
                    .followedkolpost();
            HomeFeedQuery.Data.Content2 content = datum.content()
    ...
    private List<TopPicksDomain> convertToTopPicksDomain(List<HomeFeedQuery.Data.Top_pick> top_picks) {
        List<TopPicksDomain> listToppicks = new ArrayList<>();
        if (top_picks != null) {
            for (HomeFeedQuery.Data.Top_pick topPick : top_picks) {
