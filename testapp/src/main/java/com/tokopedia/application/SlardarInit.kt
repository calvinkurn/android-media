...
object SlardarInit {
    ...
    fun initApm(context: Context) {
        ...
        initBuilder.maxValidLaunchTimeMs(15 * 1000)
        initBuilder.enableDeviceInfoOnPerfData(true)
        initBuilder.evilMethodThresholdMs(5);
        initBuilder.viewIdMonitorPageSwitch(true)
        initBuilder.pageTraceListener { pageClassName, show ->
            Log.d("LogDeva", "Page $pageClassName show status: $show")
        }
        initBuilder.debugMode(true)
        initBuilder.fullFpsTracer(true)
