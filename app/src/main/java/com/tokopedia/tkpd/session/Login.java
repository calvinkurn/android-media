...
import com.tokopedia.tkpd.fragment.FragmentSecurityQuestion;
...
import com.tokopedia.tkpd.session.presenter.SessionImpl;
        ...
        switch (session.getWhichFragment()) {
            case TkpdState.DrawerPosition.LOGIN:
                if (isFragmentCreated(LOGIN_FRAGMENT_TAG)) {
                    ...
                    if (intent != null) {
                        mEmail = intent.getStringExtra(com.tokopedia.tkpd.session.presenter.Login.EXTRA_EMAIL);
                        GoToIndex = intent.getBooleanExtra(com.tokopedia.tkpd.session.presenter.Login.GO_TO_INDEX_KEY, false);
                    }
