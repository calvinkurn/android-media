package com.tokopedia.tkpd.addtocart.presenter;
...
import com.tokopedia.tkpd.addtocart.interactor.AddToCartNetInteractor;
import com.tokopedia.tkpd.addtocart.interactor.AddToCartNetInteractorImpl;
...
import com.tokopedia.tkpd.addtocart.utils.NetParamUtil;
...
import com.tokopedia.tkpd.shipping.interactor.KeroNetInteractor;
import com.tokopedia.tkpd.shipping.interactor.KeroNetInteractorImpl;
import com.tokopedia.tkpd.shipping.model.kero.Attribute;
import com.tokopedia.tkpd.shipping.model.kero.Data;
import com.tokopedia.tkpd.shipping.network.KeroppiParam;
...
public class AddToCartPresenterImpl implements AddToCartPresenter {
    ...
    private List<Attribute> filterAvailableKeroShipment(List<Attribute> datas, List<Shipment> oldShipments) {
        ...
        for (Attribute shipment : datas) {
            for(Shipment oldShipment : oldShipments)
            {
                if(shipment.getShipperId().equals(oldShipment.getShipmentId())){
                    List<com.tokopedia.tkpd.shipping.model.kero.Product> packages = filterAvailableKeroPackageShipment(shipment
                            .getProducts(), oldShipment.getShipmentPackage());
    ...
    private List<com.tokopedia.tkpd.shipping.model.kero.Product> filterAvailableKeroPackageShipment(List<com.tokopedia.tkpd.shipping.model.kero.Product> datas, List<ShipmentPackage> dataOld) {
        List<com.tokopedia.tkpd.shipping.model.kero.Product> packageResults = new ArrayList<>();
        for (com.tokopedia.tkpd.shipping.model.kero.Product packageShipment : datas) {
            for(ShipmentPackage shipmentPackage : dataOld)
            {
                if (packageShipment.getShipperProductId().equals(shipmentPackage.getSpId())) packageResults.add(packageShipment);
            }
        }
    ...
    @Override
    public void calculateKeroAddressShipping(@NonNull Context context, @NonNull final OrderData orderData) {
        CommonUtils.dumper("rates/v1 kerorates called calculateKeroAddressShipping");
        keroNetInteractor.calculateKeroCartAddressShipping(context, AuthUtil.generateParamsNetwork(context,
                KeroppiParam.paramsKeroOrderData(orderData)), new KeroNetInteractor.OnCalculateKeroAddressShipping() {
                    @Override
                    public void onSuccess(List<Attribute> datas) {
                        viewListener.renderFormShipmentRates(filterAvailableKeroShipment(datas, orderData.getShipments() ));
                    }
    ...
    public void calculateAllPrices(@NonNull Context context, @NonNull final OrderData orderData) {
        if ((orderData.getShipment() != null
            ...
            keroNetInteractor.calculateKeroCartAddressShipping(context, AuthUtil.generateParamsNetwork(context,
                    KeroppiParam.paramsKeroOrderData(orderData)),
    ...
    public void updateAddressShipping(@NonNull final Context context,
        ...
        addToCartNetInteractor.updateAddress(context, maps,
                new AddToCartNetInteractor.OnUpdateAddressListener() {
                    ...
                    @Override
                    public void onError() {
                        viewListener.showErrorMessage(context.getString(R.string.default_request_error_unknown));
                    }
                });
    }
    ...
    public void sendAnalyticsATCSuccess(@NonNull Context context,
        ...
        values.put(context.getString(R.string.value_price_total), orderData.getPriceTotal().replace("Rp", "").replace(".", "").trim());
        values.put(context.getString(R.string.value_price_per_item), orderData.getPriceItem().replace("Rp", "").replace(".", "").trim());
        ...
        PaymentTracking.atcLoca(productCartPass, orderData, values);
    ...
    public void sendAppsFlyerATC(@NonNull Context context, @NonNull OrderData orderData) {
        ...
        values.put(AFInAppEventParameterName.PRICE, CurrencyFormatHelper.convertRupiahToInt(orderData.getPriceItem()));
