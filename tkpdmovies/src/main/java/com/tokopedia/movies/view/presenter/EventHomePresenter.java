...
import com.tokopedia.movies.domain.GetEventsListByLocationRequestUseCase;
import com.tokopedia.movies.domain.GetEventsListRequestUseCase;
import com.tokopedia.movies.domain.GetSearchEventsListRequestUseCase;
import com.tokopedia.movies.domain.model.EventsCategoryDomain;
import com.tokopedia.movies.domain.model.EventsItemDomain;
...
public class EventHomePresenter extends BaseDaggerPresenter<EventsContract.View> implements EventsContract.Presenter {
    ...
    private List<CategoryViewModel> convertIntoCategoryListVeiwModel(List<EventsCategoryDomain> categoryList) {
        ...
        if (categoryList != null) {
            for (EventsCategoryDomain moviesCategoryDomain : categoryList
                    ) {
                if ("top".equalsIgnoreCase(moviesCategoryDomain.getName())) {
                    categoryViewModels.add(0, new CategoryViewModel(moviesCategoryDomain.getTitle(), moviesCategoryDomain.getName(), convertIntoCategoryListItemsVeiwModel(moviesCategoryDomain.getItems())));

                } else {
                    categoryViewModels.add(new CategoryViewModel(moviesCategoryDomain.getTitle(), moviesCategoryDomain.getName(), convertIntoCategoryListItemsVeiwModel(moviesCategoryDomain.getItems())));
    ...
    public List<CategoryItemsViewModel> convertIntoCategoryListItemsVeiwModel(List<EventsItemDomain> categoryResponseItemsList) {
        ...
        if (categoryResponseItemsList != null) {
            ...
            for (EventsItemDomain categoryEntity : categoryResponseItemsList
                ...
                CategoryItemsViewModel.setCensor(categoryEntity.getCensor());
                CategoryItemsViewModel.setCategoryId(categoryEntity.getCategoryId());
                CategoryItemsViewModel.setDuration(categoryEntity.getDuration());
                ...
                CategoryItemsViewModel.setCustomText2(categoryEntity.getCustomText2());
                CategoryItemsViewModel.setGenre(categoryEntity.getGenre());
//                CategoryItemsViewModel.setSchedules(categoryEntity.getSchedules());
