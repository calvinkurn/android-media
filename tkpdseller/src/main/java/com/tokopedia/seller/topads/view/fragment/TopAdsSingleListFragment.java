...
public class TopAdsSingleListFragment extends TopAdsListFragment<TopAdsSingleListAdapter, TopAdsSingleListPresenter> {
    ...
    @Override
    protected boolean getActionOnSelectedMenu(ActionMode actionMode, MenuItem menuItem) {
        int itemId = menuItem.getItemId();
        if (itemId == R.id.action_delete) {
            presenter.actionDeleteAds(adapter.getSelectedAds(multiSelector.getSelectedPositions()));
            return true;
        }else if(itemId == R.id.action_edit){
            return true;
        }else if (itemId == R.id.action_off){
            presenter.actionOffAds(adapter.getSelectedAds(multiSelector.getSelectedPositions()));
            return true;
        }else if(itemId == R.id.action_on){
            presenter.actionOnAds(adapter.getSelectedAds(multiSelector.getSelectedPositions()));
            return true;
        }
        return false;
    }
