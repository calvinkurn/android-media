...
package com.tokopedia.seller.gmstat.views.williamchart.chart.animation.easing;
...
public class ElasticEase extends BaseEasingMethod {
	...
	protected float easeOut(float time) {
		...
		float s = p / 4;
		return (a *
				  (float) Math.pow(2, -10 * time) *
				  (float) Math.sin((time * 1 - s) * (2 * (float) Math.PI) / p) + 1);
	}
	...
	protected float easeInOut(float time) {
		...
		if (pos < 1.f) return -.5f *
				  ((float) Math.pow(2.f, 10.f * (pos -= 1.f)) *
							 (float) Math.sin((pos * 1f - s) * (2.f * Math.PI) / p));
